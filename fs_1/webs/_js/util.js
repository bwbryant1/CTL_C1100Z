...
// For LAN subnets Name allow space.
function isValidNameAllowSpace(name) {
   var i = 0;	
   
   for ( i = 0; i < name.length; i++ ) {
      if ( isNameUnsafe(name.charAt(i)) == true && name.charAt(i) != " ")
         return false;
   }
...
function isPasswordUnsafeDDNS(compareChar) {
   var unsafeString = "\"<>'&|;+`$()#";
...
function isValidPasswordDDNS(pwd){
   var i = 0;

   for ( i = 0; i < pwd.length; i++ ) {
      if ( isPasswordUnsafeDDNS(pwd.charAt(i)) == true )
         return false;
   }
   return true;
}