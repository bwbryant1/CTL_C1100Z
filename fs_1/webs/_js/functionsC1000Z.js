...
var isLinkStausTimerWork = false;
var isDateTimerWork = false;
var isItselfTimerWork = false;
...
function getonlinetime_task()
{
	...
	if (window.XMLHttpRequest)  {
		...
		DateTimer = setTimeout('getonlinetime_task()', 10000);
		isDateTimerWork = true;
		return;
	}
...
function updateConnectionStatus (req)
{
	...
	if ( ConnectionStatus != "" && ConnectionStatus != null ) {
		...
		//linespeeddown|linespeedup|broadban|ispstats
			...
			if(ConnectionStatusArray[2]=='ethWanUp' || ConnectionStatusArray[2]=='ethWanDown')
			{
				$('.broadbandDSL').hide();
				$('.broadbandWAN').show();
				if (ConnectionStatusArray[2] == 'ethWanUp')
					document.getElementById("broadbandWAN").innerHTML = '<img src="../_images/connectionStatus_Connected.png" alt="Connected" width="152" height="25" />'+'<p class="connected">' + ConnectionStatusArray[0] + '/' + ConnectionStatusArray[1] + ' Mbps</p>';
				else
					document.getElementById("broadbandWAN").innerHTML = '<img src="../_images/connectionStatus_notConnected.png" alt="Not Connected" width="152" height="25" /><p class="notconnected">N/A Mbps</p>';
			}
			else if (ConnectionStatusArray[2]=='Showtime' || ConnectionStatusArray[2]=='Idle')
			{
				$('.broadbandWAN').hide();
				$('.broadbandDSL').show();
				if (ConnectionStatusArray[2] == 'Showtime')
					document.getElementById("broadbandDSL").innerHTML = '<img src="../_images/connectionStatus_Connected.png" alt="Connected" width="152" height="25" />'+'<p class="connected">' + ConnectionStatusArray[0] + '/' + ConnectionStatusArray[1] + ' Mbps</p>';
				else
					document.getElementById("broadbandDSL").innerHTML = '<img src="../_images/connectionStatus_notConnected.png" alt="Not Connected" width="152" height="25" /><p class="notconnected">N/A Mbps</p>';
			}
			else
			{
				$('.broadbandWAN').hide();
				$('.broadbandDSL').show();
				document.getElementById("broadbandDSL").innerHTML = '<img src="../_images/connectionStatus_Connecting.png" alt="Connecting" width="152" height="25" /><p class="notconnected">N/A Mbps</p>';
			}
...
// done hiding -->
	...
	LinkStausTimer = setTimeout('updateConnectionStatus_take()', 10000);
	isLinkStausTimerWork = true;
...
function WriteMenuHeader(id, supportConsoleStatus)
{
   ...
   document.writeln("<li class='modemstatus' onclick=\'cleanTimer()\'><a href='modemstatus_connectionstatus.html'"+m_id+">Connection Status</a></li>");
   document.writeln("<li class='quicksetup' onclick=\'cleanTimer()\'><a href='quicksetup.html'"+q_id+">Quick Setup</a></li>");  
   document.writeln("<li class='wirelesssetup' onclick=\'cleanTimer()\'><a href='wirelesssetup_basicsettings.html'"+w_id+">Wireless Setup</a></li>");
   document.writeln("<li class='utilities' onclick=\'cleanTimer()\'><a href='utilities_reboot.html'"+u_id+">Utilities</a></li>");
   document.writeln("<li class='advancedsetup' onclick=\'cleanTimer()\'><a href='advancedsetup_accessscheduler.html'"+a_id+">Advanced Setup</a></li>");
   if(id=='supportconsole' || supportConsoleStatus=='1')
   {
      document.writeln("<li class='supportconsole' onclick=\'cleanTimer()\'><a href='supportconsole_wanvlans.html'"+s_id+">Support</a></li>");
   }
	document.writeln("</ul>");  
}
...
function WriteModemStatusMenuHeader(id, l2Type, isHpnaSupport)
{
   ...
   document.writeln("<li onclick=\'cleanTimer()\'><a href=\"modemstatus_connectionstatus.html\" " + conn_id + ">Connection Status</a></li>");
   if(l2Type != 3) {
      document.writeln("<li onclick=\'cleanTimer()\'><a href=\"modemstatus_dslstatus.html\" " + dsl_id + ">DSL Status</a></li>");
   } else {
      document.writeln("<li onclick=\'cleanTimer()\'><a href=\"modemstatus_wanstatus.html\" " + wan_id + ">WAN Status</a></li>");
   }
   document.writeln("<li onclick=\'cleanTimer()\'><a href=\"modemstatus_internetstatus.html\" " + i_id + ">Internet Status</a></li>");
   document.writeln("<li onclick=\'cleanTimer()\'><a href=\"modemstatus_ethernetstatus.html\" " + e_id + ">Ethernet Status</a></li>");
   document.writeln("<li onclick=\'cleanTimer()\'><a href=\"modemstatus_wifistatus.wl?wlSsidIdx=0\" " + w_id + ">Wireless Status</a></li>");
   if(isHpnaSupport == '1') {
      document.writeln("<li onclick=\'cleanTimer()\'><a href=\"modemstatus_hpnastatus.html\" " + h_id + ">HPNA Status</a></li>");
   }
   document.writeln("<li onclick=\'cleanTimer()\'><a href=\"modemstatus_firewallstatus.html\" " + f_id + ">Firewall Status</a></li>");
   document.writeln("<li onclick=\'cleanTimer()\'><a href=\"modemstatus_landevicelist.html\" " + d_id + ">Device Table</a></li>");
   document.writeln("<li onclick=\'cleanTimer()\'><a href=\"modemstatus_nattable.html\" " + n_id + ">NAT Table</a></li>");
   document.writeln("<li onclick=\'cleanTimer()\'><a href=\"modemstatus_routingtable.html\" " + rut_id + ">Routing Table</a></li>");
   document.writeln("<li onclick=\'cleanTimer()\'><a href=\"modemstatus_iptrafficqos.html\" " + q_id + ">QoS Table</a></li>");
   document.writeln("<li onclick=\'cleanTimer()\'><a href=\"modemstatus_routertable.html\" " + res_id + ">Resource Table</a></li>");
}

function WriteConnectionStatus(dslStatus, dslStatusLine2, wanStatus, downRate, upRate, isBond, downRateLine2, upRateLine2)
{
	document.writeln('<h2 class="broadbandDSL">CenturyLink<sup>&trade;</sup> DSL</h2><span id="broadbandDSL" class="broadbandDSL">');
	if (dslStatus == 'Showtime' || dslStatus=='ethWanUp')
	{
	...
	else if (dslStatus == 'Idle' || dslStatus=='ethWanDown')
	{
	...
	document.writeln('<h2 class="broadbandWAN">CenturyLink<sup>&trade;</sup> WAN</h2><span id="broadbandWAN" class="broadbandWAN">');
	if (dslStatus == 'Showtime' || dslStatus=='ethWanUp')
	{
		document.writeln('<img src="../_images/connectionStatus_Connected.png" alt="Connected" width="152" height="25" />');
		document.writeln('<p class="connected">'+ downRate + '/' + upRate + ' Mbps</p>');
	...
	if(dslStatus=='ethWanUp' || dslStatus=='ethWanDown')
	{
		$('.broadbandDSL').hide();
		$('.broadbandWAN').show();
	}
...
//__MSTC__, Loujia 0904 add Lan subnet page E_
   ...
   document.writeln("<li onclick=\'cleanTimer()\'><a href=\"advancedsetup_accessscheduler.html\" " + block_mainId + ">Blocking/Filtering</a>");
   document.writeln("  <ul>");
   document.writeln("    <li onclick=\'cleanTimer()\'><a href=\"advancedsetup_accessscheduler.html\" " + accessscheduler_id + ">Access Scheduler</a></li>");
   document.writeln("    <li onclick=\'cleanTimer()\'><a href=\"advancedsetup_servicesblocking.html\" " + servicesblocking_id + ">Service Blocking</a></li>");
   document.writeln("    <li onclick=\'cleanTimer()\'><a href=\"advancedsetup_websiteblocking.html\" " + websiteblocking_id + ">Website Blocking</a></li>");
   document.writeln("  </ul>");
   document.writeln("</li>");
   document.writeln("<li onclick=\'cleanTimer()\'><a href=\"advancedsetup_broadbandsettings.html\" " + broadbandsettings_id + " id=\"double\">Broadband Settings </a></li>");

   document.writeln("<li onclick=\'cleanTimer()\'><a href=\"advancedsetup_wanipaddress.html\" " + wan_settings + ">WAN Settings</a>");
   document.writeln("  <ul>");
   document.writeln("    <li onclick=\'cleanTimer()\'><a href=\"advancedsetup_wanipaddress.html\" " + wanipaddress_id + ">WAN Settings</a></li>");
   //document.writeln("    <li><a href=\"advancedsetup_wanipv6address.html\" " + IPv6wansettings_id + ">IPv6 WAN Settings</a></li>");
   document.writeln("    <li onclick=\'cleanTimer()\'><a href=\"advancedsetup_dynamicdns.html\" " + dynamicdns_id + ">Dynamic DNS</a></li>");
   document.writeln("  </ul>");
   document.writeln("</li>");

   document.writeln("<li onclick=\'cleanTimer()\'><a href=\"advancedsetup_dhcpsettings.html\" " + lan_settings + ">LAN Settings </a>");
   document.writeln("  <ul>");
   document.writeln("    <li onclick=\'cleanTimer()\'><a href=\"advancedsetup_dhcpsettings.html\" " + dhcpsettings_id + ">DHCP Settings</a></li>");
   //document.writeln("    <li><a href=\"advancedsetup_ipv6lansettings.html\" " + IPv6lansettings_id + ">IPv6 LAN Settings</a></li>");
   document.writeln("    <li onclick=\'cleanTimer()\'><a href=\"advancedsetup_dhcpreservation.html\" " + dhcpreservation_id + ">DHCP Reservation</a></li>");
   document.writeln("    <li onclick=\'cleanTimer()\'><a href=\"advancedsetup_dnshostmapping.html\" " + dnshostmapping_id + ">DNS Host Mapping</a></li>");
//__MSTC__, Loujia 0904 add Lan subnet page S_
   document.writeln("    <li onclick=\'cleanTimer()\'><a href=\"advancedsetup_lansubnet.html\" " + lansubnet_id + ">LAN Subnets</a></li>");	
//__MSTC__, Loujia 0904 add Lan subnet page E_
   //document.writeln("    <li><a href=\"_html/advancedsetup_VLAN.html\" " + VLAN_id + ">VLAN</a></li>");
   document.writeln("  </ul>");
   document.writeln("</li>");
   document.writeln("<li onclick=\'cleanTimer()\'><a href=\"advancedsetup_qos.html\" " + qos_id + ">QoS</a></li>");
   document.writeln("<li onclick=\'cleanTimer()\'><a href=\"advancedsetup_remotegui.html\" " + remote_mainId + " id=\"double\">Remote Management </a>");
   document.writeln("  <ul>");
   document.writeln("    <li onclick=\'cleanTimer()\'><a href=\"advancedsetup_remotegui.html\" " + remotegui_id + ">Remote GUI</a></li>");
   document.writeln("    <li onclick=\'cleanTimer()\'><a href=\"advancedsetup_remotetelnet.html\" " + remotetelnet_id + ">Remote Console </a></li>");
   document.writeln("  </ul>");
   document.writeln("</li>");
   document.writeln("<li onclick=\'cleanTimer()\'><a href=\"advancedsetup_dynamicrouting.html\" " + routing_mainId + ">Routing</a>");
   document.writeln("  <ul>");
   document.writeln("    <li onclick=\'cleanTimer()\'><a href=\"advancedsetup_dynamicrouting.html\" " + dynamicrouting_id + ">Dynamic Routing</a></li>");
   document.writeln("    <li onclick=\'cleanTimer()\'><a href=\"advancedsetup_staticrouting.html\" " + staticrouting_id + ">Static Routing</a></li>");
   document.writeln("  </ul>");
   document.writeln("</li>");
   document.writeln("<li onclick=\'cleanTimer()\'><a href=\"advancedsetup_admin.html\" " + security_mainId + ">Security</a>");
   document.writeln("  <ul>");
   document.writeln("    <li onclick=\'cleanTimer()\'><a href=\"advancedsetup_admin.html\" style=\"white-space:nowrap;overflow:hidden;word-break:keep-all;\"" + admin_id + ">Administrator Password</a></li>");
   document.writeln("    <li onclick=\'cleanTimer()\'><a href=\"advancedsetup_applications.html\" style=\"white-space:nowrap;overflow:hidden;word-break:keep-all;\"" + applications_id + ">Application Forwarding</a></li>");
   document.writeln("    <li onclick=\'cleanTimer()\'><a href=\"advancedsetup_advancedportforwarding.html\" " + advancedportforwarding_id + ">Port Forwarding </a></li>");
   document.writeln("    <li onclick=\'cleanTimer()\'><a href=\"advancedsetup_dmzhosting.html\" " + dmzhosting_id + ">DMZ Hosting</a></li>");
   document.writeln("    <li onclick=\'cleanTimer()\'><a href=\"advancedsetup_ipv4firewall.cgi\" " + ipv4firewall_id + ">IPv4 Firewall</a></li>");
   //document.writeln("    <li><a href=\"advancedsetup_ipv4firewall.html\" " + ipv4firewall_id + ">IPv4 Firewall</a></li>");
   document.writeln("    <li onclick=\'cleanTimer()\'><a href=\"advancedsetup_ipv6firewall.html\" " + ipv6firewall_id + ">IPv6 Firewall</a></li>");
   //document.writeln("    <li><a href=\"_html/advancedsetup_IPSec.html\" " + IPSec_id + ">IPSec</a></li>");
   document.writeln("    <li onclick=\'cleanTimer()\'><a href=\"advancedsetup_nat.html\" " + nat_id + ">NAT</a></li>");
   document.writeln("    <li onclick=\'cleanTimer()\'><a href=\"advancedsetup_upnp.html\" " + upnp_id + ">UPnP</a></li>");
   document.writeln("    <li onclick=\'cleanTimer()\'><a href=\"advancedsetup_sipalg.html\" " + sipalg_id + ">SIP ALG</a></li>");
   document.writeln("  </ul>");
   document.writeln("</li>");
}
...
function WriteSupportConsoleMenuHeader(id, isWifi5GSupport, isVoiceSupport)
{
   var wanVlan_id='', vlanstaticroutes_id='', tr069_id='', imagehistory_id='', upgradeimage_id='', igmp_id='',
       ...
       voip_basic_id='', voip_line1_id='', voip_line2_id='', voip_sipclient_id='', voip_diag_id='', communitywifi_id='';
   ...
   switch (id){
      ...
      case 'communitywifi': communitywifi_id = "class=\"selected\""; break;
   ...
   document.writeln("<li onclick=\'cleanTimer()\'><a href=\"supportconsole_wanvlans.html\" " + wanVlan_id + ">WAN VLANs</a></li>");
   document.writeln("<li onclick=\'cleanTimer()\'><a href=\"supportconsole_vlanstaticroutes.html\"  " + vlanstaticroutes_id + " id=\"double\">VLAN Static Routes</a></li>");
   document.writeln("<li onclick=\'cleanTimer()\'><a href=\"supportconsole_tr069.html\" " + tr069_id + ">TR-069</a></li>");
   document.writeln("<li onclick=\'cleanTimer()\'><a href=\"supportconsole_imagehistory.html\" " + imagehistory_id + ">Upgrade History</a></li>");
   document.writeln("<li onclick=\'cleanTimer()\'><a href=\"supportconsole_upgradeimage.html\" " + upgradeimage_id + ">Upgrade Image</a></li>");
   document.writeln("<li onclick=\'cleanTimer()\'><a href=\"supportconsole_igmp.html\" " + igmp_id + ">IGMP</a></li>");
   document.writeln("<li onclick=\'cleanTimer()\'><a href=\"supportconsole_communitywifi.html\" " + communitywifi_id + ">Community WiFi</a></li>");
   document.writeln("<li onclick=\'cleanTimer()\'><a href=\"supportconsole_oampingtest.html\" " + oampingtest_id + ">OAM Ping Test</a></li>");
   document.writeln("<li onclick=\'cleanTimer()\'><a href=\"supportconsole_dnscache.html\" " + dnscache_id + ">DNS Cache</a> </li>");
   document.writeln("<li onclick=\'cleanTimer()\'><a href=\"supportconsole_toolbox.html\" " + toolbox_id + ">Tool Box</a> </li>");
   document.writeln("<li onclick=\'cleanTimer()\'><a href=\"supportconsole_smartled.html\" " + smartled_id + ">Smart LED</a> </li>");
   document.writeln("<li onclick=\'cleanTimer()\'><a href=\"supportconsole_modemoverview.html\" " + modemoverview_id + ">Modem Overview</a> </li>");
   if(isWifi5GSupport == '1') {
      document.writeln("<li onclick=\'cleanTimer()\'><a href=\"supportconsole_5G_basicsettings.html\" " + wlan5g_basic_id + ">5G Prism Settings</a>");
      document.writeln("  <ul>");
      document.writeln("    <li onclick=\'cleanTimer()\'><a href=\"supportconsole_5G_basicsettings.html\" " + wlan5g_basic_id + ">5G Basic Settings</a></li>");
      document.writeln("    <li onclick=\'cleanTimer()\'><a href=\"supportconsole_5G_wireless5g.html\" " + wlan5g_security_id + ">5G Wireless Security</a></li>");
      document.writeln("    <li onclick=\'cleanTimer()\'><a href=\"supportconsole_5G_devicetable.html\" " + wlan5g_devicetable_id + ">5G Prism STB List</a></li>");
      document.writeln("    <li onclick=\'cleanTimer()\'><a href=\"supportconsole_5G_wps.html\" " + wlan5g_wps_id + ">5G Prism WPS</a></li>");
      document.writeln("    <li onclick=\'cleanTimer()\'><a href=\"supportconsole_5G_radiosettings.html\" " + wlan5g_radio_id + ">5G Radio Settings</a></li>");
      document.writeln("    <li onclick=\'cleanTimer()\'><a href=\"supportconsole_5G_prismlinkstatus.html\" " + wlan5g_linkstatus_id + ">5G Prism Link Status</a></li>");
      document.writeln("  </ul>");
   }
   if(isVoiceSupport == '1') {
      document.writeln("<li onclick=\'cleanTimer()\'><a href=\"supportconsole_voip_basicsettings.html\" " + voip_basic_id + ">VoIP Settings</a>");
      document.writeln("  <ul>");
      document.writeln("    <li onclick=\'cleanTimer()\'><a href=\"supportconsole_voip_basicsettings.html\" " + voip_basic_id + ">Basic VoIP Settings</a></li>");
      document.writeln("    <li onclick=\'cleanTimer()\'><a href=\"supportconsole_voip_line1features.html\" " + voip_line1_id + ">Line 1 Features</a></li>");
      document.writeln("    <li onclick=\'cleanTimer()\'><a href=\"supportconsole_voip_line2features.html\" " + voip_line2_id + ">Line 2 Features</a></li>");
      document.writeln("    <li onclick=\'cleanTimer()\'><a href=\"supportconsole_voip_sipclient.html\" " + voip_sipclient_id + ">SIP Client</a></li>");
      document.writeln("    <li onclick=\'cleanTimer()\'><a href=\"supportconsole_voip_diagnostics.html\" " + voip_diag_id + ">Diagnostics</a></li>");
      document.writeln("  </ul>");
      document.writeln("</li>");
   }
}
...
function isPwdformatPass(val) {
   ...
   for ( i = 0; i < len; i++ ) 
   {
      ...
      if(!finduppercase && uppercasecharacter.indexOf(val.substring(i,i+1)) != -1)
      {
         finduppercase = 1; //finf  uppercase
      }
      if(!findlowercase && lowercasecharacter.indexOf(val.substring(i,i+1)) != -1)
      {
         findlowercase = 1; // find  lowercase
      }
      if(!finenumber && number.indexOf(val.substring(i,i+1)) != -1)
      {
         finenumber = 1; // find  number
      }
      if(!findspecialcharacter && specialcharacter.indexOf(val.substring(i,i+1)) != -1)
      {
         findspecialcharacter = 1; //find  findspecialcharacter
      }
    
	   
   }

   if(finduppercase && findlowercase && finenumber && findspecialcharacter)
   {	    
      return true;
   }
...
function isPwdNoSeqPass(val) {
   var len     = val.length;
   var i       = 0;
   var j       = 0;

   var seqformat =new Array("0123456789","9876543210","QWERTYUIOP","qwertyuiop",
   	                        "POIUYTREWQ","poiuytrewq","ASDFGHJKL","asdfghjkl",
   	                         "LKJHGFDSA","lkjhgfdsa","ZXCVBNM","zxcvbnm",
   	                         "MNBVCXZ","mnbvcxz");  
   ...
   for ( i = 0; i < (len - 3); i++ ) 
   {
      ...
      for(j = 0; j < 14 ; j++)
      {
         if(seqformat[j].indexOf(val.substring(i,i+4)) != -1)
         {
            return false; //find  sequence
         }
...
function cleanTimer()
{
   if(isLinkStausTimerWork)
   {
      clearTimeout(LinkStausTimer);
      isLinkStausTimerWork = false;
   }
   if(isDateTimerWork)
   {
   ...
   if(isItselfTimerWork)
   {
      clearTimeout(ItselfTimer);
      isItselfTimerWork = false;
   }
   return;
}
