...
function updateConnectionInfo (req)
{
	...
	if ( ConnectionInfo != "" && ConnectionInfo != null ) {
		...
		//linespeeddown|linespeedup|PPPTYPE|USERNAME|broadban|ispstats|Encap|WANIP|dns1|dns2|session time|PPPDEXIST|lineStatus
			...
			if (ConnectionInfos[20] == 1) //ATM
			{
				document.getElementById("ENCAPSULATION").innerHTML = ConnectionInfos[21];
			}
			else if (ConnectionInfos[20] == 2) //PTM
			{
				document.getElementById("ENCAPSULATION").innerHTML = ConnectionInfos[22];
			}
			else if (ConnectionInfos[20] == 3) //ETH
			{
				document.getElementById("ENCAPSULATION").innerHTML = 'Ethernet - ' + ConnectionInfos[23];
			}
			else
			{
				document.getElementById("ENCAPSULATION").innerHTML = 'N/A';
			}
...
//__CTLK__, Kaiping
			if (ConnectionInfos[4] == 'Showtime' || ConnectionInfos[17] == 'Showtime')
				document.getElementById("broadband").innerHTML = '<font color="#015F2C"><strong>'+'CONNECTED'+'</strong></font>';
			else if (ConnectionInfos[4] == 'Idle' && ConnectionInfos[17] == 'Idle')
