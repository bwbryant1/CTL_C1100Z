...
extern int MinBadBlockSet; /* max bad block number */
extern int MaxBadBlockSet; /* min bad block number */
#endif
		...
		for (i = 0; i < len; i++) {
			...
			for (j = 0; j < 8; j += bits, act += 2) {
				...
				if ((rootfs_start_addr <= (((offs << 2) + (act >> 1)) << this->bbt_erase_shift) &&
					...
					if(flash_rootfs_bad_block_number > 0){
						rootfs_end_addr = ((unsigned int)simple_strtoul(pTag->rootfsAddress, NULL, 10) + BOOT_OFFSET + 
									 (unsigned int)simple_strtoul(pTag->rootfsLen, NULL, 10)) - FLASH_BASE + flash_rootfs_bad_block_number * TAG_BLOCK_LEN_NAND;
					}
				}
#endif
...
from, bd->options, mtd64_ll_low(startblock), mtd64_ll_low(numblocks));
	...
	rootfs_end_block = getNandBlock(((unsigned int)simple_strtoul(pTag->rootfsAddress, NULL, 10) + BOOT_OFFSET + 
									 (unsigned int)simple_strtoul(pTag->rootfsLen, NULL, 10)), 0);
#endif
	for (i = startblock; i < numblocks;) {
        ...
        /* Assume there have bad blocks between MinBadBlockSet ~ MaxBadBlockSet */
        if((MinBadBlockSet != -1) && (MaxBadBlockSet != -1))
        {
	        if((i >> 1 >= MinBadBlockSet) && (i >> 1 <= MaxBadBlockSet))
	        {
		        /* Setting the ret to "1", bad block marked */
		        ret = 1;
	        }
        }
#endif
...
int brcmnand_displayBBT(struct mtd_info* mtd)
{
	struct brcmnand_chip *this = mtd->priv;
 ...
 */
/* Support the CLI command to test incorrect bad block tables(BBT), __CHT__, MitraStar SeanLu, 20140328 */
void brcmnand_preprocessKernelArg(struct mtd_info *mtd)
{
	struct brcmnand_chip *this = mtd->priv;
...
#if 1	/*ChingLung@MSTC the roofs is not complete when the bad block infomation is not same as BBT */
...
//printk("%s: gClearBBT=erase|eraseall, start=%08x, end=%08x\n", __FUNCTION__, bOffsetStart, bOffsetEnd);
			...
			bOffset = bOffset + mtd->erasesize) 
	{
		...
		/* 
		 * CPE still need to check this bad block is correct or incorrect 
		 */
		  
#if 0 /* Fix the BBT and OOB value not match, __CHT__, MitraStar SeanLu, 20140328 */
...
#if 1 /* Fix the BBT and OOB value not match, __CHT__, MitraStar SeanLu, 20140328 */
		else {
#else
		{
#endif
...
if (gdebug && res!=0) printk("########## %s: read_oob returns %d\n", __FUNCTION__, res);
				...
				if (!res) {
					if (check_short_pattern (oobbuf, this->badblock_pattern)) {
						isBadBlock = 1;
#if 1//__CTLK__,JhenYang, 2014-04-02
						if (mtd->block_isbad(mtd, offs)) 
						{
							printk(KERN_INFO "Found bad block at offset %0llx, in BBT\n", offs);
							isBadBlock = 0;
							break;
						}
#endif
...
PRINTK("Found bad block at offset %0llx\n", offs);
					...
					{
 						/* CPE check the OOB, this is not a bad block*/
						if (mtd->block_isbad(mtd, offs)) 
						{
							/* However, the BBT bad block marked, we must rebuild the BBT */
							printk(KERN_INFO "$$$$$$$$$$$$$$$$$$$$$$$$$$$ NOT match as offset %0llx $$$$$$$$$$$$$$$$$$$$$$$$$\n", offs);
							printk(KERN_INFO "Rebuilding the BBT in %s\n", __FUNCTION__);	
							/* Not match, clean the BBT*/
...
//gdebug=4;
		...
		case NANDCMD_RESCAN:
			if (isBadBlock) {
				...
				flash_rootfs_bad_block_number = finfo.flash_rootfs_bad_block_number;
				rootfs_start_addr = ((unsigned int)simple_strtoul(pTag->rootfsAddress, NULL, 10) + BOOT_OFFSET) - FLASH_BASE;
				rootfs_end_addr = ((unsigned int)simple_strtoul(pTag->rootfsAddress, NULL, 10) + BOOT_OFFSET + 
									(unsigned int)simple_strtoul(pTag->rootfsLen, NULL, 10)) - FLASH_BASE + flash_rootfs_bad_block_number * TAG_BLOCK_LEN_NAND;
					
				printk(KERN_INFO "flash_rootfs_bad_block_number= %d\n", flash_rootfs_bad_block_number);
				printk(KERN_INFO "rootfs_start_addr= 0x%08x\n", rootfs_start_addr);
				printk(KERN_INFO "rootfs_end_addr= 0x%08x\n", rootfs_end_addr);

				if(rootfs_start_addr<=(unsigned int)bOffset && (unsigned int)bOffset<=rootfs_end_addr)
				{
					flash_rootfs_bad_block_number++;
					printk(KERN_INFO "flash_rootfs_bad_block_number= %d\n", flash_rootfs_bad_block_number);
					kerSysFlashBadBlockNumSet(flash_rootfs_bad_block_number);
...
#if 1 /* Fix the BBT and OOB value not match, __CHT__, MitraStar SeanLu, 20140328 */
	/* Setting the gClearBBT to rescan, CPE will rescan BBT when bootup*/
	gClearBBT = NANDCMD_RESCAN;
	PRINTK("@@@@@@@@@@@@@@@@@@@@@@ %s: gClearBBT = %d\n", __FUNCTION__, gClearBBT);
#endif
