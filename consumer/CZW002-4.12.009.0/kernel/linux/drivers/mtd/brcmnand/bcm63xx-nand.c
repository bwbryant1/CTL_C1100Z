...
static struct mtd_partition bcm63XX_nand_parts[] = 
{
    ...
    {name: "app",    offset: 0, size: 0},
#endif
#if 1 // __CTLK__, Richard Huang, CERT
    {name: "cert",    offset: 0, size: 0},
#endif
...
/* Support the CLI command to test incorrect bad block tables(BBT), __CHT__, MitraStar SeanLu, 20140328 */
...
#if 1//__MTS__, RaynorChung: Support 963268 nand flash, patch form SVN#3597 on http://svn.zyxel.com.tw/svn/CPE_SW1/BCM96368/trunk/P-870HA/branches/cht/fttb8/4.11
        /* __MSTC__, zongyue: additional partition change from 3 to 4 */
        nandinfo->nr_parts = 5;
#else
        nandinfo->nr_parts = 4;
#endif
...
#if 1 /* __MSTC__, zongyue: additional partition */
        /* App */
        bcm63XX_nand_parts[3].offset = nvram.AppNandPartOfsKb * 1024;
#if 1 // __CTLK__, Richard Huang, CERT
        bcm63XX_nand_parts[3].size = (nvram.AppNandPartSizeKb - 4096) * 1024;
#else
        bcm63XX_nand_parts[3].size = nvram.AppNandPartSizeKb * 1024;
#endif
        bcm63XX_nand_parts[3].ecclayout = mtd->ecclayout;
#endif

#if 1 // __CTLK__, Richard Huang, CERT
        bcm63XX_nand_parts[4].offset = bcm63XX_nand_parts[3].offset + bcm63XX_nand_parts[3].size;
        bcm63XX_nand_parts[4].size = 4 * 1024 * 1024; // 4MB
