...
#ifdef CONFIG_MITRASTAR_SIP_ALG_SESSION_TRACK //__CTLK__, brianlin
static struct list_head sipalg_list;
//static rwlock_t sipalg_sess_lock = RW_LOCK_UNLOCKED;
DEFINE_SPINLOCK(sipalg_sess_lock);
static int process_sipAlgSessUpdate(const char *callid, struct nf_conn *ct, enum ip_conntrack_info ctinfo, struct nf_conntrack_man *addr);
static int  sipalg_sess_show(char *buf, char **bloc, off_t off, int length, int *eof, void *data);
#endif 
...
#ifdef CONFIG_MITRASTAR_SIP_ALG_SESSION_TRACK //__CTLK__, brianlin
static int parse_sip_callID(char **begin, char *end, char *callid)
{
	char *p = *begin;
	char *pStart = p;
	int ret = -1;
	...
	while (p < end) {
		...
		if ((*p == '\r') && (*(p+1) == '\n')) {
			strncpy(callid, pStart, p - pStart);
			ret = 0;
			break;
		}
		if (*p == ' ') {
...
static int process_connection(struct sk_buff *skb, unsigned int protoff,
			      ...
			      int *dlen, struct nf_conntrack_man *addr, char *callid)
#else
static int process_connection(struct sk_buff *skb, unsigned int protoff,
			      struct nf_conn *ct,
			      enum ip_conntrack_info ctinfo, char **dptr,
...
#endif
{
	...
	*addr = a;
	...
	/* add SIP ALG session */
	if( a.l3num == AF_INET )
		process_sipAlgSessUpdate(callid, ct, ctinfo, &a);
#endif
...
#ifdef CONFIG_MITRASTAR_SIP_ALG_SESSION_TRACK //__CTLK__, brianlin
static int process_callID(char **dptr, int *dlen, char *callid)
{
	int ret = NF_ACCEPT;
	char *p = *dptr;

	if (!find_field(&p, *dptr+*dlen, POS_CALLID))
		return ret;
	
	if (parse_sip_callID(&p, *dptr+*dlen, callid)) {
		return NF_DROP;
	}
	pr_debug("process_callID: Call-ID=%s\n", callid);
...
static int process_sipAlgSessUpdate(const char *callid, struct nf_conn *ct, enum ip_conntrack_info ctinfo, struct nf_conntrack_man *addr)
{
	...
	struct sip_alg_session_track_s *track, *track_new;

	spin_lock_bh(&sipalg_sess_lock);
	list_for_each(pos, &sipalg_list) {
		track = list_entry(pos, struct sip_alg_session_track_s, list);
		if( strncmp(callid, track->callID, strlen(track->callID)) == 0) {
			pr_debug("process_sipAlgSessUpdate: matched Call-ID=%s\n", callid);
			found = 1;
			break;
		}
	}
	...
	if(found) {
		...
		if (!memcmp(&ct->tuplehash[dir].tuple.src.u3,
			    &ct->tuplehash[!dir].tuple.dst.u3,
		    	sizeof(ct->tuplehash[dir].tuple.src.u3))) {
			track->dstIP = addr->u3.ip;
			pr_debug("process_sipAlgSessUpdate: update srcIP=%X\n", track->dstIP);
		}
		/* LAN->WAN situations */
		else { 
			track->srcIP = addr->u3.ip;
			pr_debug("process_sipAlgSessUpdate: update dstIP=%X\n", track->srcIP);
		}		
	}
	else {
		track_new = kmalloc(sizeof(struct sip_alg_session_track_s), 0); 
		memset(track_new, 0x0, sizeof(struct sip_alg_session_track_s));
		strcpy(track_new->callID, callid);

		/* WAN->LAN situations */
		if (!memcmp(&ct->tuplehash[dir].tuple.src.u3,
			    &ct->tuplehash[!dir].tuple.dst.u3,
		    	sizeof(ct->tuplehash[dir].tuple.src.u3))) {
			track_new->dstIP = addr->u3.ip;
			pr_debug("process_sipAlgSessUpdate: add srcIP=%X\n", track_new->dstIP);
		}
		/* LAN->WAN situations */
		else { 
			track_new->srcIP = addr->u3.ip;
			pr_debug("process_sipAlgSessUpdate: add dstIP=%X\n", track_new->srcIP);
		}

		list_add(&(track_new->list), &sipalg_list);
	}
	spin_unlock_bh(&sipalg_sess_lock);

	return ret;
}

static int process_sipAlgSessRemove(const char *callid)
{
	int ret = -1;
	struct list_head *pos;
	struct sip_alg_session_track_s *track;

	spin_lock_bh(&sipalg_sess_lock);
	list_for_each(pos, &sipalg_list) {
		track = list_entry(pos, struct sip_alg_session_track_s, list);
		if( strncmp(callid, track->callID, strlen(track->callID)) == 0) {
			pr_debug("process_sipAlgSessRemove: matched Call-ID=%s\n", callid);
			list_del(pos);
			kfree(track);
			ret = 0;
			break;
...
static int  sipalg_sess_show(char *buf, char **bloc, off_t off, int length, int *eof, void *data)
{
	...
	list_for_each(pos, &sipalg_list) {
		...
		len += sprintf(tmp, "%s %x %x \r\n", track->callID, track->srcIP, track->dstIP);
		strcat(buf, tmp);
	}
	spin_unlock_bh(&sipalg_sess_lock);
...
#ifdef CONFIG_MITRASTAR_SIP_ALG_SESSION_TRACK //__CTLK__, brianlin
	/* Process Call-ID field */
	pr_debug("nf_conntrack_sip: process_call-ID\n");
	memset(callid, 0x0, sizeof(callid));
	ret = process_callID(&dptr, &datalen, callid);
	if (ret != NF_ACCEPT)
		goto out;

	/* Process BYE/Cancel/3xx/4xx/5xx/6xx  */
	if (strlen(callid) && (memcmp(dptr, "BYE", sizeof("BYE") - 1) == 0 ||
		memcmp(dptr, "CANCEL", sizeof("CANCEL") - 1) == 0 ||
		memcmp(dptr, "SIP/2.0 3", sizeof("SIP/2.0 3") - 1) == 0 ||
		memcmp(dptr, "SIP/2.0 4", sizeof("SIP/2.0 4") - 1) == 0 ||
		memcmp(dptr, "SIP/2.0 5", sizeof("SIP/2.0 5") - 1) == 0 ||
	    memcmp(dptr, "SIP/2.0 6", sizeof("SIP/2.0 6") - 1) == 0)) {
		pr_debug("nf_conntrack_sip: process_sipSessRemove\n");
		process_sipAlgSessRemove(callid);
	}
#endif
...
#ifdef CONFIG_MITRASTAR_SIP_ALG_SESSION_TRACK //__CTLK__, brianlin
	remove_proc_entry("sipalg_sess_show", init_net.proc_net);
#endif
}
...
#ifdef CONFIG_MITRASTAR_SIP_ALG_SESSION_TRACK //__CTLK__, brianlin
	INIT_LIST_HEAD(&sipalg_list);
	proc = create_proc_read_entry("sipalg_sess_show", S_IRUGO, init_net.proc_net, sipalg_sess_show, NULL);
	if (proc == NULL)
		printk("Failed to create sipalg_sess_show\r\n");
#endif 
	return 0;
}
