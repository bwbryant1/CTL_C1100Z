...
/* Support LAN port match criteria, __TELEFONICA__, ZyXEL YungAn, 20090819. */
		...
		else if (strcasecmp(token, "start_reservation") == 0)
			read_ip(line, &cur_iface->start_reservation);
		else if (strcasecmp(token, "end_reservation") == 0)
			read_ip(line, &cur_iface->end_reservation);
		else if (strcasecmp(token, "auto_reservation") == 0)
			read_yn(line, &cur_iface->auto_reservation);		
#endif
		...
		for (i = 0; i < iface->max_leases; i++) {
			if (iface->leases[i].yiaddr != 0) {
				...
				fwrite(iface->leases[i].hostname,
				       ...
				       1, fp);
#if 1 // __CTL__, KJ
				fwrite(iface->leases[i].vendorid,
				       sizeof(iface->leases[i].vendorid),
				       1, fp);
#endif
			}
		}
...
/* TR-098 DHCP Conditional Serving Pool, __TELEFONICA__, ZyXEL YungAn, 20090817. */
   ...
   // Auto-detect the type of  LAN devices, Option 60
   if((cmsUtl_strcmp(lease->vendorid, "MSFT_IPTV") == 0) ||
      (cmsUtl_strcmp(lease->vendorid, "IPTV_STB") == 0)){
      snprintf(body->hostType, sizeof(body->hostType), MDMVS_IPTVSTB);
	  snprintf(body->hostName, sizeof(body->hostName), "IPTV STB");
   }
   else if(cmsUtl_strcmp(lease->vendorid, "Xbox 360") == 0){
      snprintf(body->hostType, sizeof(body->hostType), MDMVS_XBOX);
      snprintf(body->hostName, sizeof(body->hostName), "Xbox 360");
   }
   else if(cmsUtl_strcmp(lease->vendorid, "PS3") == 0){
      snprintf(body->hostType, sizeof(body->hostType), MDMVS_PS3);
	  snprintf(body->hostName, sizeof(body->hostName), "PS3");
   }
   else{
      snprintf(body->hostType, sizeof(body->hostType), MDMVS_COMPUTER); // default value
   }
#if 1 //__CTLK__, Thief
   if(!IS_EMPTY_STRING(lease->vendorid)){
      strcpy(body->vendorClassID, lease->vendorid);
   }
   if(!IS_EMPTY_STRING(lease->classid)){
      strcpy(body->userClassID, lease->classid);
   }
   if(!IS_EMPTY_STRING(lease->clientid)){
      strcpy(body->clientID, lease->clientid);
   }
#endif
#else
   snprintf(body->addressSource, sizeof(body->addressSource), MDMVS_DHCP);
...
#if 1 // __CTLK__, TengChang, refer to Q1000Z 
   //Fix when the inferface is condservpoolx_brx in DHCP conditional serving
   ...
   cmsNet_getPortNameFromMac(cur_iface->interface, lease->chaddr, intfName);
#endif
   strcpy(body->incomingPort, intfName);
