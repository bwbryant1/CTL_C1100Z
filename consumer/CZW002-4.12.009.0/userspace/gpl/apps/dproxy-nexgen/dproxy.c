...
#if 1 // __CTLK__, Richard Huang, Dnsproxy create entries without limitation cause CPE oom, Merge from CHT
#define MAX_DNS_LIST 512
#endif
...
next_wan:
  ...
  dns_dyn_hosts_add();
   ...
   errorCodeFailover = 0;
	...
	int dnsserverPref = 0; //0:no pref, use querytype	1:ipv4 first 2: ipv6 first
#endif
...
void dns_handle_new_query(dns_request_t *m)
{
  ...
  if( query_type == A || query_type == AAA)
#else
   if( query_type == A )
#endif
   {
...
#if 1 // __CTLK__, Richard Huang, Dnsproxy create entries without limitation cause CPE oom, Merge from CHT
           if(dns_list_count >= MAX_DNS_LIST)
           {
              dns_request_t *ptr;

              ptr = dns_request_last;
              dns_list_remove(ptr);

              debug("dns count=[%d] is maximum, need to delete oldest one\n", dns_list_count);
           }
#endif

           cmsLog_notice("Found dns %s for subnet %s", dns1, srcIP);
#if 1 //__CTLK__, JhihPing, bugfix CDRouter-ipv6_dns_45,46
           /* when failover, we don't add dns list again */
           if ( m->errorCode == 0 ) {
              dns_list_add(m);
           }
#else
...
#if 1 //__CTLK__, JhihPing, /* CPE first sends query with source address "::1" */
      ...
      if( m->message.header.flags.flags & 0x8000)
      {
         ...
         if ( !ptr->errorCode && 
            ((m->message.header.flags.f.rcode  == 2) ||
             (m->message.header.flags.f.rcode == 4) ||
             (m->message.header.flags.f.rcode == 5)) )
         {
            ptr->errorCode = 1;
            errorCodeFailover = 1;
