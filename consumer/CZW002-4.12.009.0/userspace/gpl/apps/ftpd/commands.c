...
void do_configBackup(char *filename)
{
    ...
    if (cmsMsg_receive(msgHandle, &responseMsg) != CMSRET_SUCCESS)
    {
	   ...
	   control_printf(SL_FAILURE, "could not receive GET_CONFIG_FILE msg from smd.");
       return;
    }      
...
void command_quit(char *params __attribute__((unused)))
{
   ...
   ret = cmsImg_writeValidatedImage(imagePtr, uploadSize, imageFormat, msgHandle, 0);
#else
...
#if 1 //__MSTC__, Nick Tseng, support config backup when FTPD storage feature is defined
	if (!strcmp(mapped, "/mnt/backupsettings.conf")) {
		do_configBackup("backupsettings.conf");
		return;
	}
#endif
