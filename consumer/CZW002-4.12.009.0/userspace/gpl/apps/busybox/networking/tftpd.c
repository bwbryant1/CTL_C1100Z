...
#if 1 //yiwei, support qtn 5GHz (detect uboot)
#define QTN_UBOOT_NOT_EXIST 0
#define QTN_UBOOT_EXIST	 1
#define QTN_UBOOT_CHECKING_POINT "/var/qtn_wifi_uboot"
#endif
...
#if 1 //yiwei, support qtn 5GHz (detect uboot)
	if(strcmp(list[0], "u-boot.bin") == 0) {
		FILE*	pQtnUd = NULL ;
		if(NULL != (pQtnUd = fopen(QTN_UBOOT_CHECKING_POINT, "w+"))){
			fprintf(pQtnUd, "%s\r\n", tsize_ret) ;
			fclose(pQtnUd) ;
		}
   	}
...
// brcm   fclose(file);
   ...
   printf("Total upload size: %d bytes\n", uploadSize);
    ...
    if ((ret = cmsImg_writeImage(imagePtr, uploadSize, msgHandle, 0)) != CMSRET_SUCCESS)
#else
