...
void main_cleanup(void)
{
	...
	if(vid!=-1){
		//Make diagnostic flow id be 255
		int flowId = 0;
		flowId = SKBMARK_SET_FLOW_ID(flowId, 255);

		sprintf(cmd, "iptables -t mangle -D OUTPUT -j MARK --set-mark 0x%x -d %s >/dev/null &",
				flowId, dstIP);
		system(cmd);
		sprintf(cmd,"vlanctl --tx --tags 0 --if %s --filter-skb-mark-flowid %d --push-tag --set-vid %d 0 --set-pbits %d 0 --rule-remove-by-filter", l2IfName, 255, vid, priorityMark);
#ifdef WGET_DEBUG
		fprintf(stderr, " ------%s\n", cmd);
#endif
		system(cmd);
	}	
#if 1 //__CTLK__, reference to P870HNU_51c_Qwest_404L01, Thief
	if(dscpMark!=-1){
		sprintf(cmd, "iptables -t mangle -D OUTPUT -j DSCP --set-dscp 0x%x -d %s", 
												dscpMark, dstIP);
		fprintf(stderr, "-------%s\n", cmd);
		system(cmd);
	}
...
enum {
	...
	WGET_OPT_L2IFNAME   = (1 << 12),
	WGET_OPT_VID        = (1 << 13),
#ifdef BRCM_CMS_BUILD
	WGET_OPT_REQEID     = (1 << 14),
#endif
	WGET_OPT_RETRIES    = (1 << 15),
	WGET_OPT_NETWORK_READ_TIMEOUT = (1 << 16),
	WGET_OPT_PASSIVE    = (1 << 17),
	WGET_OPT_HEADER     = (1 << 18) * ENABLE_FEATURE_WGET_LONG_OPTIONS, 
	WGET_OPT_POST_DATA  = (1 << 19) * ENABLE_FEATURE_WGET_LONG_OPTIONS, 
#endif
};

static void NOINLINE retrieve_file_data(FILE *dfp, int output_fd, int ignoreWrite)
{
	char buf[512];
...
#if 1 /*__MSTC__, Justin*/			
			if ( (ignoreWrite==0) && safe_fwrite(buf, 1, n, output_fd) != n)
			{
				main_cleanup();
...
int wget_main(int argc UNUSED_PARAM, char **argv)
{
	...
	opt = getopt32(argv, "csqO:P:Y:U:n:i:wd:M:I:v:R:",
				&fname_out,  &dir_prefix, &proxy_flag, &user_agent, &sessionNumber, &ifName,  &dscp, &ethernetPriority,&l2IfName, &vidStr, &requestEid
	...
	if (opt & WGET_OPT_ETNERPRIORITY)
		priorityMark = atoi(ethernetPriority);
	...
	if (!(opt & WGET_OPT_OUTNAME)) {
		...
		if (sNum > 1){
			...
			output_fd = xfopen(cmd, (opt & WGET_OPT_CONTINUE ? "a" : "w"));
		}
#endif
	} else {
		...
		else
		{
		    if (sNum == 1)
		    {
			    output_fd = xfopen(fname_out, (opt & WGET_OPT_CONTINUE ? "a" : "w"));
		    }
			else
			{
				sprintf(cmd, "%s_%d", fname_out, multiID);
 ...
 resolve_lsa:
    ...
    if(vid!=-1){
		...
		sprintf(cmd, "iptables -t mangle -A OUTPUT -j MARK --set-mark 0x%x -d %s >/dev/null &",
				flowId, dstIP);
		printf("%s\n", cmd);
		system(cmd);
		sprintf(cmd,"vlanctl --tx --tags 0 --if %s --filter-skb-mark-flowid %d --push-tag --set-vid %d 0 --set-pbits %d 0 --rule-insert-before 0", l2IfName, 255, vid, priorityMark);
#ifdef WGET_DEBUG
	    fprintf(stderr, "-----------------%s\n", cmd);
#endif
	    system(cmd);
...
#if 1 //__CTLK__, reference to P870HNU_51c_Qwest_404L01, Thief
	if(dscpMark!=-1){
		sprintf(cmd, "iptables -t mangle -I OUTPUT -j DSCP --set-dscp 0x%x -d %s", 
												dscpMark, dstIP);
		fprintf(stderr, "-----------------%s\n", cmd);
		system(cmd);
	}
...
#if 1//__CTLK__,JhenYang
	retrieve_file_data(dfp, output_fd, ignoreWrite);
#else
...
#ifdef WGET_DEBUG
	fprintf(stderr, "ifBytesReceivedStart:%ul\r\n", ifBytesReceivedStart);
	fprintf(stderr, "ifBytesReceivedEnd:%ul\r\n", ifBytesReceivedEnd);
#endif
	return;
}
#if 1 //__MSTC__, Justin, Support TR143
