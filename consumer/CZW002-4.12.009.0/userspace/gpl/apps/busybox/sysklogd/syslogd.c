...
#if 1 //__CTLK__,ShuFang
#define LOGFILESIZE 500
#endif

#if 1 //__CTLK__, LingChun, Please use /etc/TZ to operate the system time
long ntp_timeoffset=0;
#endif
...
#if 1 //__CTLK__, ShuFang
void WriteLogToFile(time_t now, char *msg)
{
#ifdef SYSLOGD_WRLOCK
		struct flock fl;
#endif
...
static void timestamp_and_log(int pri, const char *msg, int len)
{
...
#if 1 //__CTLK__, ShuFang
	struct tm *tmdata;
	char logtofilebuf[MAX_READ*2 + 128];
	char timestamp_buff[32];
		time(&now);
#if 1 //__CTLK__, LingChun, Please use /etc/TZ to operate the system time
	now = now - ntp_timeoffset;
#endif		
	tmdata = gmtime(&now);
	...
	if (option_mask32 & OPT_small)
		...
		#if 1 //__CTLK__, ShuFang
		sprintf(logtofilebuf, "%s %.64s %s %s\n", timestamp, G.hostname, res, msg);
		#endif
	}

	/* Log message locally (to file or shared mem) */
	#if 1 //__CTLK__, ShuFang
	WriteLogToFile(now, logtofilebuf);
	#endif
...
#endif
#if ENABLE_FEATURE_REMOTE_LOG
#if 1 //__CTLK__, JhihPing, /* support SIP log server */
        /* the remote log is only for VoIP, only sends VoIP, kernel, reboot message to a server */
        if ((strstr(recvbuf, "[VoIP]") != NULL) ||
            /*(strstr(recvbuf, "kernel") != NULL) ||*/
            (strstr(recvbuf, "Device Reboot") != NULL))
        {
#endif
...
#if 1 //__CTLK__, LingChun, Please use /etc/TZ to operate the system time
int syslogd_reload_timeoffset(){
	struct timeval ntp_time;
	
	char *objValue;
	SINT32 count;
	
	if ((objValue = cmsMem_alloc(BUFLEN_16, ALLOC_ZEROIZE)) == NULL)
	{
	   cmsLog_error("malloc failed");
	   return 0;
	}
	
	count = cmsPsp_get("ntp_Timeoffset", objValue, BUFLEN_16);
	if (count == 0)
	{
	...
	else if (count != BUFLEN_16)
	{
	   cmsLog_error("error while reading ntp_UTCtime data from scratch pad, count=%d", count);
	   cmsMem_free(objValue);
	   return 0;
	}	
	
	if(cmsUtl_strcmp(objValue, "")){
		 ntp_timeoffset = atol(objValue);
	}
	cmsMem_free(objValue);
	return 0;
}
...
#if 1 //__CTLK__, LingChun, Please use /etc/TZ to operate the system time
	sprintf(cmd, "echo '%d' > /var/run/syslogd.pid",getpid());	
	system(cmd);
	syslogd_reload_timeoffset();
	signal(SIGUSR1,syslogd_reload_timeoffset);	
#else
	//umask(0); - why??
