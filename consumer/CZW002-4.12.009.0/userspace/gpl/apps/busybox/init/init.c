...
#define FILESIZEINFLASH 1024
#endif


#if 1 //__CTLK__, ShuFang, refer to PK5001Z
/* save log or clear log here */
#include <stdio.h>
#include <string.h>
#include <fcntl.h>
...
void copyLogToFlash(char *logFilePath, char *flashLogFilePath, unsigned int logFileSize, int logFileRotate)
{
	...
	//If there is no any log stored in flash, copy /log/system.log to /home/log/system0.log
	FILE *dummyFP;
	char log_0_file[128];
	...
	if((dummyFP = fopen(logFilePath,"r")) == NULL){
		...
		printf("remove old flash file and save new log file\n");
		sprintf(cmd,"rm %s%d.log", flashLogFilePath, 0);
		system(cmd);
		sprintf(cmd,"cp %s %s%d.log", logFilePath, flashLogFilePath, 0);
		system(cmd);		
		fclose(dummyFP);
		...
		//Copy /home/log/system0.log to /log/temp.log
		sprintf(cmd,"cp %s%d.log %s", flashLogFilePath, 0, tempFile);
		system(cmd);

		//Calculate the remained size of /log/temp.log (/home/log/system0.log)
		...
		//Copy back /log/temp.log to /home/log/system0.log
		...
		if(index >= logRemainSize){
			...
			//Copy /log/temp.log to /home/log/system0.log
			sprintf(cmd,"cp %s %s%d.log", tempFile, flashLogFilePath, 0);
			system(cmd);
		}
		//Remove /log/temp.log
...
#if 1 //__CTLK__, ShuFang
/* save log or clear log here */
int checkLogSaveStatus() 
{
	FILE *fp;
	char buf[8] = {0};
	int ret = 0;
	fp = fopen("/var/SaveInFlash.log", "r");
...
#if 1 //__CTLK__, ShuFang, refer to PK5001Z
	syslog(LOG_CRIT,"Device Reboot.\n");
	/* save log or clear log here */
	message(L_CONSOLE | L_LOG, "checkLogSaveStatus:%d\r\n", checkLogSaveStatus());
	if (checkLogSaveStatus() == 1)
		copyLogToFlash("/var/log/syslog", "/app/sysall", FILESIZEINFLASH*1024, 2);
	else
	{
		if(access("/app/sysall0.log",F_OK)==0){
			unlink("/app/sysall0.log");
		}
		if(access("/app/sysall1.log",F_OK)==0){
					unlink("/app/sysall1.log");
		}
	}
#endif
