...
#endif
#if 1 //__CTLK__, Thief
static CmsRet flashImage(char *imagePtr, UINT32 imageLen, CmsImageFormat format, void* msgHandle, UINT8 setAct);
#else
...
#if 1 //__CTLK__, Thief
CmsRet flashImage(char *imagePtr, UINT32 imageLen, CmsImageFormat format, void* msgHandle, UINT8 setAct)
#else
    ...
    if (format == CMS_IMAGE_FORMAT_BROADCOM || format == CMS_IMAGE_FORMAT_FLASH)
    {
        if ( !sysGetTAGFromFlash( &curtagData ) ){
            ...
            if (!sysGetTAGFromFlashByPartition( &tagData1, 1)) {
                fwSeq[0] = strtoul(tagData1.imageSequence, NULL, 10);
            }
            if (!sysGetTAGFromFlashByPartition( &tagData1, 2)) {
                fwSeq[1] = strtoul(tagData1.imageSequence, NULL, 10);
            }

            if (fwSeq[0] == strtoul(curtagData.imageSequence, NULL, 10)) {
                nextimage = '2';
            } else if (fwSeq[1] == strtoul(curtagData.imageSequence, NULL, 10)) {
                nextimage = '1';
            }

            if(setAct != 0) {
                // Upgrade to Active partition
                // if nextimage == 2, change it to 1
                // if nextimage == 1, change it to 2
                nextimage = (nextimage == '1')? '2':'1';
            }
#else
            if ( !sysGetTAGFromFlashByPartition( &tagData1, 1 ) ) {
...
#if 1 // __CTLK__, Richard Huang
        sprintf(str, "%d|%c|", setAct, nextimage);
#else
		...
		if(eid == EID_TR69C){
			strcat(str, "ACS|");
#if 1 // __CTLK__, Richard Huang
 			snprintf(handle, sizeof(handle), "%d|ACS", setAct);
#else
			strcpy(handle, "ACS");
#endif
		}
		else if (eid == EID_HTTPD || eid == EID_HTTPD_SSL || eid == EID_FTPD){
			strcat(str, "LAN|");
#if 1 // __CTLK__, Richard Huang
			snprintf(handle, sizeof(handle), "%d|LAN", setAct);
#else
			strcpy(handle, "LAN");
#endif
		}
			...
			strcat(str, "Recovery|");
#if 1 // __CTLK__, Richard Huang
			snprintf(handle, sizeof(handle), "%d|Recovery", setAct);
#else
			strcpy(handle, "Recovery");
#endif
		}
 ...
 */
#if 1 // __CTLK__, Richard Huang
CmsRet cmsImg_writeImage(char *imagePtr, UINT32 imageLen, void *msgHandle, UINT8 setAct)
#else
...
#endif
{
   ...
   {
#if 1 // __CTLK__, Richard Huang
      ret = cmsImg_writeValidatedImage(imagePtr, imageLen, format, msgHandle, setAct);
#else
...
#if 1 // __CTLK__, Richard Huang
CmsRet cmsImg_writeValidatedImage(char *imagePtr, UINT32 imageLen, CmsImageFormat format, void *msgHandle, UINT8 setAct)
#else
...
#if 1 //__CTLK__, Thief
      ret = flashImage(imagePtr, imageLen, format, msgHandle, setAct);
#else
...
#if 1 // __CTLK__, SeanLu 20150417, Support check IPv6 address of multiple LANs
      while (cmsNet_getIfAddr6("br", addrIdx,
...
CmsRet cmsImg_changeBootPartition()
{
   CmsRet ret = CMSRET_SUCCESS;
   ret = devCtl_boardIoctl(BOARD_IOCTL_CHANGE_BOOT_PARTITION, 0, "", 0, 0, 0);
   return (ret);
}
#endif
