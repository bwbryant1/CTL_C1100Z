...
CmsRet cmsNet_getGloballyUniqueIfAddr6(const char *ifname, char *ipAddr, UINT32 *prefixLen)
{
   ...
   while (CMSRET_SUCCESS == ret)
   {
      ...
      {
#if 1 //__CTLK__, SeanLu 20150108, normalization IPv6 address
         /* normalization IPv6 address, ex: fd00:0000:0000:0000:0000:0000:0000:0001 -> fd00::1 */
         struct in6_addr in6Addr;
         memset(&in6Addr, 0 , sizeof(struct in6_addr));
         if (inet_pton(AF_INET6, ipAddr, &in6Addr) <= 0)
         {
            cmsLog_error("Invalid address=%s", ipAddr);
            return CMSRET_INVALID_ARGUMENTS;
         }
         if (inet_ntop(AF_INET6, &in6Addr, ipAddr, BUFLEN_48) == NULL)
         {
...
#if 1 //__CTLK__, JhihPing, Fix Coverity defects
...
#ifdef DMP_X_404A03_IPV6_1 //__CTLK__, Eric, refer to Q1000Z
CmsRet cmsNet_getMacFromIpAddr(const struct in6_addr ipaddr, const char *brName, unsigned char *macaddr)
#else
