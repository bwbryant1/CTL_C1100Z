...
typedef enum
{
   ...
   CMSRET_DU_UPDATE_VERSION_EXISTED= 9032,/**< Invalid Deployment Unit Update, version already exists
                                                    * (associated with ChangeDUState:update)
                                                    */
#if 1 //__CTL__,Wister refer to Q1000Z
...
#if 1 //__MTS__, TengChang, for firmwareUpgrade.html
   CMSRET_INVALID_MODELID = 9856,
   CMSRET_INVALID_BOOTROMSIZE = 9857,
#endif //__MTS__, TengChang, for firmwareUpgrade.html
} CmsRet;
...
#
#define CMS_MAX_DEFAULT_GATEWAY     8  //!< max default gateways allowed in L3 X_404A03_DefaultConnectionServices
...
typedef enum
{
   ...
   CMS_CONNECTION_MODE_UNKNOWN=3,
#endif
} ConnectionModeType;
   
#ifndef VAH_PROTOTYPE //__CTLK__,JhenYang,vlanautohunt
#define VAH_PROTOTYPE 1
enum{
   VAH_PROTO_PPPOE=1,
   VAH_PROTO_PPPOA=2,
   VAH_PROTO_DHCP=3,
   VAH_PROTO_STATICIP=4,
   VAH_PROTO_BRIDGE=5,
   VAH_LAST
};
#endif
...
#if 1//__QWEST__,Hsiao-Win
typedef struct{
    int port;
    char interface[128];
    char addr[32];
} SocketInfo;
#define UDPECHOCFG_PATH "/var/udpecho.cfg"
#define UDPECHOCONF_PATH "/var/udpecho.conf"
