...
extern char *UtilGetChipName(char *buf, int len);
...
int BcmSpiReserveSlave2(int busNum, int slaveId, int maxFreq, int spiMode, int ctrlState)
{
    ...
    if(BpGetBoardId( baseBoardIdStr ))
    {
        printk(KERN_ERR "Get Base Board ID error.\n");
        return( SPI_STATUS_ERR );
    }
    if( 0 == strcmp(baseBoardIdStr, "96818EGG")  &&  0 == strcmp(UtilGetChipName(chipname,BRCM_MAX_CHIP_NAME_LEN), "6818A0")  )
    {
        busNum=LEG_SPI_BUS_NUM;
    }
