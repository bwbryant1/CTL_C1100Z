...
#define FAP_BPM_FREE_CACHE_SIZE           1024
#define FAP_BPM_FREE_CACHE_ALLOC_THRESH   512
#define FAP_BPM_FREE_CACHE_BULK_ALLOC_MAX 512

#define FAP_BPM_EXT_FREE_CACHE_SIZE       512   
...
typedef struct {
    ...
    uint32 freeCacheBufCnt;        /* number of buffers available in freeCache*/
    uint32 *freeCachePtr;       /* the pointer to active local free cache */
    uint32  freeCacheAllocWait; /* 1=request sent to host and waiting for response */
    uint32 free;                /* count of number of buffers freed */
#if defined(CC_FAP_BPM_EXT_FREE)
    uint16 extFreeListWrIdx;
    uint16 extFreeListRdIdx;
    uint16 extFreeCurWrIdx;
#endif
} fap4keDspram_bpm_t;
...
#endif
    uint32 freeCacheBufAddr[FAP_BPM_FREE_CACHE_BULK_ALLOC_MAX];
#if defined(CC_FAP_BPM_EXT_FREE)
    uint32 extFreeCache[FAP_BPM_EXT_FREE_MAX_REQ][FAP_BPM_EXT_FREE_CACHE_SIZE];
#endif
} fap4keSdram_bpm_t;
...
typedef struct {
     ...
     */
    uint32 freeCache[FAP_BPM_FREE_CACHE_SIZE];
} fap4kePsm_bpm_t;
