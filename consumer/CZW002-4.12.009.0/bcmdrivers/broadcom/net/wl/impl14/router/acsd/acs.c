...
#if 1 // __CenturyLink__, Hong-Yu
static chanspec_t
acs_pick_chanspec_default(acs_chaninfo_t* c_info, int bw, int logSecond)
#else
static chanspec_t
...
#endif
{
	...
	if (logSecond) {
		min_score = 5000;
		second_chspec = 0;
		/* select second chanspec */
		int tmp_sb=0;
		int best_c=0;
		int second_c=1;				
		for (i = 0; i < c_info->c_count[bw]; i++) {	
			if ((candi[index].chspec & WL_CHANSPEC_BW_MASK) == WL_CHANSPEC_BW_40){
				tmp_sb = candi[index].chspec & WL_CHANSPEC_CTL_SB_MASK;
				best_c = (candi[index].chspec & WL_CHANSPEC_CHAN_MASK) + ((tmp_sb == WL_CHANSPEC_CTL_SB_LOWER) ? (-2) : (2));
				tmp_sb = candi[i].chspec & WL_CHANSPEC_CTL_SB_MASK;
				second_c = (candi[i].chspec & WL_CHANSPEC_CHAN_MASK) + ((tmp_sb == WL_CHANSPEC_CTL_SB_LOWER) ? (-2) : (2));
				if(best_c==second_c)
					continue;
			}
			
			if (!candi[i].valid || i == index)
				continue;
		...
		if (second >= 0)
			second_chspec = candi[second].chspec;
	}
#endif
...
#if 1 // __CenturyLink__, Hong-Yu
void
acs_select_chspec(acs_chaninfo_t *c_info, int logSecond)
#else
void
...
#if 1 // __CenturyLink__, Hong-Yu
			selected = a_pol->chan_selector(c_info, bw, logSecond);
#else
...
#if 1 // __CenturyLink__, Hong-Yu
	if (logSecond) {
		ACSD_PRINT("second channel spec: 0x%4x\n", second_chspec);
		c_info->second_chspec = second_chspec;
	}
#endif
}
