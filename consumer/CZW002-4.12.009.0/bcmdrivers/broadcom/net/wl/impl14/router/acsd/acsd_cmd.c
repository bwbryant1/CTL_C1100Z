...
acsd_proc_cmd(acsd_wksp_t* d_info, char* buf, uint rcount, uint* r_size)
{
	...
	*c++ = '\0';
	...
	if (!strcmp(buf, "csscan") || (!strcmp(buf, "autochannel")) || (!strcmp(buf, "csscan_bestsecond"))) {
#else
...
#if 1 // __CenturyLink__, Hong-Yu
		if (!strcmp(buf, "csscan_bestsecond")) {
			acs_select_chspec(c_info, 1);
			*r_size = sprintf(buf, "%x %x", c_info->selected_chspec, c_info->second_chspec);
		} else
