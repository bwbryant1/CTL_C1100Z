 ...
 *
 * $Id: acsd_svr.h 356787 2012-09-13 23:42:59Z $
 */
...
/* predefined policy index */
#define ACS_POLICY_LEGACY 		1
#define ACS_POLICY_INTF			2
#define ACS_POLICY_INTF_BUSY	3
#define ACS_POLICY_OPTIMIZED	4
#define ACS_POLICY_CUSTOMIZED1	5
#define ACS_POLICY_CUSTOMIZED2	6
#define ACS_POLICY_FCS			7
#define ACS_POLICY_USER			8
...
typedef chanspec_t (*acs_selector_t)(struct acs_chaninfo * c_info, int bw, int logSecond);
#else
...
typedef struct acs_fcs_s {
	...
	uint8 acs_ics_dfs;			/* enable/disable DFS chan as first chan */
...
#if 1 // __CenturyLink__, Hong-Yu
extern void acs_select_chspec(acs_chaninfo_t *c_info, int logSecond);
#else
extern void acs_select_chspec(acs_chaninfo_t *c_info);
#endif
extern int acs_scan_timer_check(acs_chaninfo_t * c_info);
...
extern int dcs_handle_request(char* ifname, wl_bcmdcs_data_t *dcs_data, uint8 mode, uint8 count);
