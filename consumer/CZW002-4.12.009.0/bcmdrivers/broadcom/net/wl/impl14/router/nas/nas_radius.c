...
#if 1 //__Qwest__, Ling-Chun, CSP
				/* for EAP-MD5 */
				if (eap->code == EAP_REQUEST && eap->length > 4 && eap->type == 4)
					sta->pae.flags |= PAE_FLAG_RADIUS_EAP_MD5;
#endif	
			}
...
#if 1 //__Qwest__, Ling-Chun, CSP
		/* EAP-MD5 */
		else if (CHECK_WPA(sta->mode) == 0 && sta->pae.flags & PAE_FLAG_RADIUS_EAP_MD5) {
			dbg(nas, "authorize %s (802.1x)", ether_etoa((char *)&sta->ea, eabuf));	
			nas_authorize(nas, &sta->ea);		
		}
#endif
		break;
