 ...
 *
 * $Id: nas_wksp.c 381893 2013-01-30 06:52:00Z $
 */
...
#ifdef DSLCPE
	printf("\t-B <duration>        RADIUS authentication failure barring time\n");
#endif
...
#ifdef DSLCPE
		case 'B':
			/* update authentication blockout retry interval */
			nwcb->nas.auth_blockout_time = strtoul(optarg, NULL, 0);
			NASDBG("nas[%d].auth_blockout_time %s\n", i, optarg);
			break;
#endif
		case 'N':
...
#if 1//LingChun, CSP#635934, Both APs can't ping to each other when the security of AP is WPA2-PSK and WPS is enabled.
...
#ifdef DSLCPE
	nwcb->nas.auth_blockout_time = (uint32) info.auth_blockout_time;
	NASDBG("new nwcb's nas auth blockout time %d\n", nwcb->nas.auth_blockout_time);
#endif
