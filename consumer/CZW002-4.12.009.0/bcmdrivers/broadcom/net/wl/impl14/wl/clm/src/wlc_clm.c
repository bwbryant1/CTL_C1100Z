 ...
 *
 * $Id: wlc_clm.c 5400 2012-08-30 06:27:03Z $
 */
...
#define FORMAT_VERSION_MAJOR 10 /* BLOB format version major number */
...
typedef struct locale_data {
	...
	/** base address of channel range descriptors */
	const clm_channel_range_t *chan_ranges;

	/** Base address of rate set definitions */
...
#define RATE_TYPE(rate_idx) ((rate_type[(unsigned)(rate_idx) >> 2] >> (((rate_idx) & 3) << 1)) & 3)
...
/** Retrieves byte string that contains locale definition
	...
	locale_data_t *loc_data)
{
	...
	loc_data->chan_ranges = (const clm_channel_range_t *)GET_DATA(ds_id, channel_ranges);
	loc_data->rate_sets = (const unsigned char *)GET_DATA(ds_id, locale_rate_sets);
	...
	if (locales->country_flags & CLM_DATA_FLAG_REG_SC_RULES_MASK) {
		...
		if (sub_chan_rules_set_80 && ((unsigned)rules_idx <
			...
			loc_data->sub_chan_channel_rules_80.channel_rules =
				(const clm_sub_chan_channel_rules_80_t *)relocate_ptr(ds_id,
				sub_chan_region_rules->channel_rules);
			loc_data->sub_chan_channel_rules_80.num = sub_chan_region_rules->num;
		}
		if (ds->has_160mhz) {
			...
			if (sub_chan_rules_set_160 && ((unsigned)rules_idx <
				...
				loc_data->sub_chan_channel_rules_160.channel_rules =
					(const clm_sub_chan_channel_rules_160_t *)relocate_ptr(
					ds_id, sub_chan_region_rules->channel_rules);
				loc_data->sub_chan_channel_rules_160.num =
	...
	clm_channels_t *valid_channels, clm_channels_t *restricted_channels)
{
	...
	if (loc_data.def_ptr) {
		get_channels(valid_channels, loc_data.valid_channels,
			loc_data.def_ptr[CLM_LOC_DSC_CHANNELS_IDX], loc_data.chan_ranges,
			loc_data.combs[CLM_BW_20]);
		get_channels(restricted_channels, loc_data.restricted_channels,
			loc_data.def_ptr[CLM_LOC_DSC_RESTRICTED_IDX], loc_data.chan_ranges,
			loc_data.combs[CLM_BW_20]);
		...
		do {
			...
			for (num_rec = (int)*tx_rec++; num_rec--; tx_rec += tx_rec_len) {
				const unsigned char *rates =
					get_byte_string(base_ht_loc_data[base_ht_idx].rate_sets,
	...
	const clm_limits_params_t *params, clm_power_limits_t *limits)
{
	...
	if ((band == CLM_BAND_2G) && (params->bw != CLM_BW_20) && subchannels[CLM_BW_20]) {
		/* 20-in-40, 2.4GHz. Channel to take DSSS limits from */
		channel_dsss = subchannels[CLM_BW_20];
	}
	my_memset(limits, (char)WL_RATE_DISABLED, sizeof(limits->limit));
	...
	for (base_ht_idx = 0; base_ht_idx < (int)ARRAYSIZE(base_ht_loc_data); ++base_ht_idx) {
		...
		const clm_channel_range_t *ranges = loc_data->chan_ranges;
		...
		/* Sequence of rate sets' definition */
		const unsigned char *rate_sets = loc_data->rate_sets;
		...
		if (limits_type == CLM_LIMITS_TYPE_CHANNEL) {
			...
			fill_actual_subchan_table(bw_to_chan, subchannels, limits_type, channel,
				loc_data->chan_ranges, comb_sets[CLM_BW_80],
				loc_data->sub_chan_channel_rules_80.channel_rules,
				loc_data->sub_chan_channel_rules_80.num,
				sizeof(clm_sub_chan_channel_rules_80_t));
		} else if (params->bw == CLM_BW_160) {
			fill_actual_subchan_table(bw_to_chan, subchannels, limits_type, channel,
				loc_data->chan_ranges, comb_sets[CLM_BW_160],
				loc_data->sub_chan_channel_rules_160.channel_rules,
				loc_data->sub_chan_channel_rules_160.num,
		...
		do {
			...
			for (num_rec = (int)*tx_rec++; num_rec--; tx_rec += tx_rec_len)
			{
				...
				if (tx_power_idx >= tx_rec_len) {
					...
					if (!valid_channel && channel_for_bw && channel_in_range(
						channel_for_bw, range, comb_set_for_bw))
					{
						valid_channel = TRUE;
					}
					...
					channel_in_range(channel_for_bw, range, comb_set_for_bw))
				{
					...
					for (num_rates = *rates++; num_rates--; rates++) {
						clm_power_t *pp = &limits->limit[*rates];
						/* Looking for minimum power */
						if ((*pp == WL_RATE_DISABLED) || (*pp > qdbm)) {
							*pp = qdbm;
						}
					}
					valid_channel = TRUE;
				}
				/* If this rule related to 20-in-40 DSSS channel? */
