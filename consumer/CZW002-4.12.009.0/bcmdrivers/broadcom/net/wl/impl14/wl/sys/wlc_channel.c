...
#if 1 //__CTLK__, LingChun, Q1/20 for CenturyLink US.
	if(!strcmp("Q1",mapped_ccode) && *mapped_regrev == 0 ){
		*mapped_regrev = wlc_cm->regrev;
		WL_NONE(("wl%d: %s: Using \"Q1/20\" instead of legacy \"Q1/0\" since we %s\n",
		         wlc->pub->unit, __FUNCTION__,
		         AP_ENAB(wlc->pub) ? "are operating as an AP" : "are newer than 4306"));		
	}
#endif
