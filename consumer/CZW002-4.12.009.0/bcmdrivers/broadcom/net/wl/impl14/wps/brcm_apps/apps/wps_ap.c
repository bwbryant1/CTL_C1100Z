 ...
 *
 * $Id: wps_ap.c 372004 2012-11-30 03:46:58Z $
 */
...
#if 1 //__CTLK__, Ling-Chun, M2D, don't need to display.
	int enr_status = wps_getProcessStates();
	if(wps_wksp->sc_mode == SCMODE_AP_ENROLLEE && enr_status==WPS_M2D){
		wpsap_update_wpslist(wps_wksp,9);
	}else{
		wpsap_update_wpslist(wps_wksp,retVal);	
	}
#endif
	return retVal;
}
...
#ifdef DSLCPE_NG
static int
wpsap_update_custom2_cred(char *ssid, char *key, char *akm, char *crypto, int oob_addenr,
	 bool b_wps_Version2, uint8 band)
{
	/*
	 * WSC 1.0 Old design : Because WSC 1.0 did not support Mix mode, in default
	 * we pick WPA2-PSK/AES up in Mix mode.  If NVRAM "wps_mixedmode" is "1"
	...
	if (oob_addenr) {
		char *p;
		TUTRACE((TUTRACE_ERR, "\nwpsap_update_custom2_cred, RF Band = %d ...\n", band));
		if (band == WPS_RFBAND_50GHZ ) {	/*5G*/
			/* get randomssid */
			if ((p = wps_get_conf("wps_randomssid_5G"))) {
				strncpy(ssid, p, MAX_SSID_LEN);
				TUTRACE((TUTRACE_INFO, "Update customized SSID (5G): %s\n", ssid));
			}

			/* get randomkey */
			if ((p = wps_get_conf("wps_randomkey_5G")) && (strlen (p) >= 8)) {
				strncpy(key, p, SIZE_64_BYTES);
				TUTRACE((TUTRACE_INFO, "Update customized Key (5G): %s\n", key));
			}
			...
			if ((p = wps_get_conf("wps_randomssid"))) {
				strncpy(ssid, p, MAX_SSID_LEN);
				TUTRACE((TUTRACE_INFO, "Update customized SSID (2.4G): %s\n", ssid));
			}
	
			/* get randomkey */
			if ((p = wps_get_conf("wps_randomkey")) && (strlen (p) >= 8)) {
				strncpy(key, p, SIZE_64_BYTES);
				TUTRACE((TUTRACE_INFO, "Update customized Key (2.4G): %s\n", key));
			}
		}
		/* Modify the crypto, if the station is legacy */
	}
...
#ifdef DSLCPE_NG
			wps_set_conf("wps_currentRFband", "2");
#endif
			break;
		case WLC_BAND_2G:
		default:
...
#ifdef DSLCPE_NG
			wps_set_conf("wps_currentRFband", "1");
#endif
			break;
		}
	}
...
#else
		wpsap_update_custom2_cred(ssid, psk, dev_akm, dev_crypto, oob_addenr, 
...
#ifdef WLAN_CENTRYLINK //__CTLK__, LingChun
#else
		if (wps_wksp->b_wps_version2 &&
		    (wps_wksp->config_state == WPS_SCSTATE_CONFIGURED)) {
...
wpsap_wksp_init(wpsap_wksp_t *ap_wksp)
{
	...
	if (ap_wksp->sc_mode == SCMODE_AP_ENROLLEE) {
		...
		ret = wpsap_start_enrollment(ap_wksp->mc, dev_pin);
	}
	else {
		char *sta_pin = wps_ui_get_env("wps_sta_pin");

		TUTRACE((TUTRACE_INFO, "init sta pin = %s\n", sta_pin));
		ret = wpsap_start_registration(ap_wksp->mc, sta_pin);
	}
...
#ifdef WLAN_CENTRYLINK //__CTLK__, LingChun, 
	TUTRACE((TUTRACE_INFO, "ap_wksp->sc_mode=%d \n",ap_wksp->sc_mode));
	if(ap_wksp->sc_mode!=SCMODE_AP_ENROLLEE ){
		wpsap_save_wps_list(ap_wksp,1);	
	}
#endif

	TUTRACE((TUTRACE_INFO, "wpsap init done\n"));
	
#ifdef MTS_WLAN_WPS_FOR_TR69 //__Qwest__, SinJia, TR069 for wps
   wpsap_update_wps_method(ap_wksp->sc_mode);

   if (wpsap_test_Ethernet_Cfg(ap_wksp->sc_mode) == 0){
      TUTRACE((TUTRACE_INFO, "RPROTO: Reject by Ethernet restricted\n"));
...
#ifdef MTS_WLAN_WPS_FOR_TR69//__Qwest__, SinJia, TR069 for wps
	if( !wpsap_checkRegistrarEnable(ap_wksp) ){	
      TUTRACE((TUTRACE_INFO, "SKIP End-Device PIN!!!!! \n"));
	  ap_eap_sm_Failure(0);
	  ap_wksp->return_code = WPS_RESULT_FAILURE;
	  wps_setProcessStates(WPS_MSG_ERR);
	  return -1;
	}  
#endif

#ifdef WLAN_CENTRYLINK //__CTLK__, LingChun, WPS Allow Mac For End-Pin
#if 1//__CTLK__, Fritz, PBC doesn't need to check MAC
	if(ap_wksp->sc_mode != SCMODE_AP_ENROLLEE && strncmp(wps_ui_get_env("wps_sta_pin"), "00000000", 8) != 0 && !wpsap_checkStaMac(ap_wksp)){
#else
	if(ap_wksp->sc_mode != SCMODE_AP_ENROLLEE && /*dev_pwd_id==WPS_DEVICEPWDID_DEFAULT &&*/ !wpsap_checkStaMac(ap_wksp)){	
#endif
      TUTRACE((TUTRACE_INFO, "SKIP End-Device PIN!!!!! \n"));
	  ap_eap_sm_Failure(0);
	  ap_wksp->return_code = WPS_RESULT_FAILURE;
	  wps_setProcessStates(WPS_MSG_ERR);
	  wpsap_save_wps_list(ap_wksp,4);	  
	  wps_ui_clear_pending();
	  return -1;
	}  
	TUTRACE((TUTRACE_INFO, "Continue..... \n"));
#endif
...
{
	if (wps_wksp) {
#ifdef WLAN_CENTRYLINK //__CTLK__, Ling-Chun
		wpsap_exit_wps_list(wps_wksp->return_code);
#endif
		ap_eap_sm_deinit();
#ifdef WPS_UPNP_DEVICE
		ap_upnp_sm_deinit();
		...
		free(wps_wksp);
#ifdef MTS_WLAN_WPS_FOR_TR69 //__Qwest__, Ling-Chun
	if((wps_initCmsMsg()) == 0){
		wps_sendUpdateConfErrCmsMsg();
		wps_cleanCmsMsg();
	}else
		printf("%s@%d wps_initCmsMsg Failure.",__FUNCTION__,__LINE__);
#endif
	}
...
#if 1//__CTLK__, LingChun
		if(WPS_M2D==wps_getProcessStates() || WPS_INIT==wps_getProcessStates()){
			wps_setProcessStates(WPS_INIT);
		}else{
			wps_setProcessStates(WPS_MSG_ERR);	
		}		
...
#if 1 //__CTLK__, LingChun
				if( wps_getProcessStates()==WPS_M2D);
				else
...
#ifdef MTS_WLAN_WPS_FOR_TR69 //__QWEST__, SinJia, TR069 for wps   
		if(wpsap_test_wps_lock_state() == 0){ 
			wps_setFileVarInt("wpsLastConfigErr", WPS_CONFIG_ERROR_SETUP_LOCKED);
			TUTRACE((TUTRACE_INFO, "AP in lock up state by wpsap_test_wps_lock_state(), deny AP configuring\n"));
			return WPS_ERR_OPEN_SESSION;
		}
#endif
	case SCMODE_AP_REGISTRAR:
...
#ifdef MTS_WLAN_WPS_FOR_TR69 //__Qwest__, SinJia, TR069 for wps
int wpsap_test_wps_lock_state(){
   int valid;
   char lock[32];
   wps_getFileVar("wpsLockedState", lock);

   if(!strcmp(lock, "")){
      valid = 1;
   }else{
      if(!strcmp(lock, "Unlocked")){
         valid = 1;
      }else if(!strcmp(lock, "LockedByLocalManagement")){
         valid = 0;
      }else if(!strcmp(lock, "LockedByRemoteManagement")){
         valid = 0;
      }else if(!strcmp(lock, "PINRetryLimitReached")){
         valid = 0;
      }
   }
...
int wpsap_test_Ethernet_Cfg(int mode){
   int valid = 1;
   
   if(mode == SCMODE_AP_REGISTRAR){
      char support_method[32];

      wps_getFileVar("wpsEthernetCfg", support_method);

      if(!strcmp(support_method, "0")){
         valid = 0;
      }else{
         valid = 1;
      }
...
void wpsap_update_wps_method(int mode){
   if(mode == SCMODE_AP_ENROLLEE){
      wps_setFileVar("wpsstatus", "ap-pin");
   }else if(mode == SCMODE_AP_REGISTRAR){

      if(!strcmp("00000000", wps_ui_get_env("wps_sta_pin"))){
         wps_setFileVar("wpsstatus", "client-pbc");
      }else{
         wps_setFileVar("wpsstatus", "client-pin");
      }
   }/*else if(mode == EModeApProxy){
      //it goes by wired registrar
      wps_setFileVar("wpsstatus", "client-pin");
   }*/
}
...
//**************************************************************************
// Function Name: wpsap_checkRegistrarEnable
// Description  : Check the value set by Wps Registrar Object from mdm
// Parameters   : None
// Returns      : None.
//**************************************************************************
int wpsap_checkRegistrarEnable(wpsap_wksp_t *bcmwps){
   //bcmwps_t *bcmwps=wps_wksp->bcmwps;

   int valid = 1;
   
   //get the current sta mac
   char curr_mac_sta[32];
   sprintf(curr_mac_sta, "%02x:%02x:%02x:%02x:%02x:%02x",
      bcmwps->mac_sta[0], bcmwps->mac_sta[1], bcmwps->mac_sta[2],
      bcmwps->mac_sta[3], bcmwps->mac_sta[4], bcmwps->mac_sta[5]);   

   int i = 0;
   for(i = 0; curr_mac_sta[i]; ++i)  {
      curr_mac_sta[i] = toupper(curr_mac_sta[i]);
   }
   ...
   char key[32];
   while (1){
      sprintf(key,"wpsRegistrarDisableMac_%d",index);
      wps_getFileVar(key ,val);

      if(!strcmp(val, curr_mac_sta)){
         valid = 0;
         break;
      }else if(!strcmp(val, "")){
         //the "0" we does not compare
         //"0" just means the record has been delete temporarily
         //There may be more records belows "0"
         break;
      }
...
#ifdef WLAN_CENTRYLINK //__CTLK__, LingChun
//**************************************************************************
// Function Name: wpsap_checkStaMac
// Description  : Check the wps sta mac is the user specify in wps gui page
// Parameters   : None
// Returns      : None.
//**************************************************************************
int wpsap_checkStaMac(wpsap_wksp_t *bcmwps){  
   char* curr_allow_mac=NULL;
   curr_allow_mac = nvram_get("wps_autho_sta_mac");
   if(!curr_allow_mac)curr_allow_mac="";

   //get the current sta mac
   char curr_mac_sta[32]={0};
   sprintf(curr_mac_sta, "%02x:%02x:%02x:%02x:%02x:%02x",
      bcmwps->mac_sta[0], bcmwps->mac_sta[1], bcmwps->mac_sta[2],
      bcmwps->mac_sta[3], bcmwps->mac_sta[4], bcmwps->mac_sta[5]);   

   TUTRACE((TUTRACE_INFO, "authorizedMacs=%d , authorizedMacs_len=%d\n",bcmwps->authorizedMacs,bcmwps->authorizedMacs_len));   
   TUTRACE((TUTRACE_INFO, "curr_allow_mac=%s , curr_mac_sta=%s \n",curr_allow_mac,curr_mac_sta));   

   //compare the mac address
   if(!strcmp(curr_allow_mac,"") || !strcmp(curr_allow_mac,"00:00:00:00:00:00")){
	  TUTRACE((TUTRACE_INFO, "curr_allow_mac=%s is invalid. Pass. \n",curr_allow_mac));   
   	  //return 0;   	
   }else if(strcasecmp(curr_allow_mac, curr_mac_sta)){
      //It is not allow mac
      TUTRACE((TUTRACE_INFO,"Invalid sta mac: %s, only allow sta mac:%s, SKIP.\n",curr_mac_sta, curr_allow_mac));
      return 0;
   }   
...
#ifdef WLAN_CENTRYLINK//__CTLK__, Ling-Chun
void wpsap_update_wpslist(wpsap_wksp_t *wps_wksp, uint32 retVal){
	char proxyStateValue[2]={0};
    if(retVal == WPS_ERR_REGISTRATION_PINFAIL){
         //device pin fail
		wps_setProcessStates(WPS_MSG_ERR);//do in wpsap_close_session()(SPRID: 100422456)
		wpsap_save_wps_list(wps_wksp, 3);
    }else if(retVal == WPS_SUCCESS){
        // printf("[WPS]wps_ap.c wps setting success\n");
		wpsap_save_wps_list(wps_wksp, 2);	
		wps_setProcessStates(WPS_OK);
	}else if(retVal == WPS_MESSAGE_PROCESSING_ERROR){
		wps_setProcessStates(WPS_MSG_ERR);
		wpsap_save_wps_list(wps_wksp, 4);
	}else if(retVal == WPS_ERR_ENROLLMENT_PINFAIL){
		 //Config AP Pin Fall
		 //Notice: aplockdown_cap of Nvram should be 0
		wps_setProcessStates(WPS_MSG_ERR);//do in wpsap_close_session()(SPRID: 100422456)
		wpsap_save_wps_list(wps_wksp, 3);
	}else if(retVal ==9) {//M2D
		wpsap_save_wps_list(wps_wksp, 9);
		//wps_setProcessStates(WPS_INIT); //don't need to do this
	}	
}
#endif

#ifdef WLAN_CENTRYLINK //__CTLK__, Ling-Chun, __Qwest__, SinJia, WPS Device List
//**************************************************************************
// Function Name: wpsap_save_wps_list
// Description  : Create a list file about the wps device list. The file will be read by GUI.
//                     Once a wps device is deeted by wps, the mac will add by this funtion.
//                     The other information will fill by other functions.
//                     wlmngr_updateWpsStaList() (wlutil.c): Detect the device is online.
//                     rutSys_updateWpsFileByHostsFile() (rut_system.c): Detect the IP and Name of the device
// Parameters   : wps_wksp_t
//                      state
// Returns      : None.
//**************************************************************************
void wpsap_save_wps_list(wpsap_wksp_t *bcmwps,int state){
   //open a wps.tmp for temporary write.
   FILE *fp_write = fopen("/var/wps.tmp", "w");
   if(fp_write==NULL){
      return;   
   }

   //Notice: if the fp_read is NULL is vaild
   FILE *fp_read = fopen("/var/wps", "r");

   //loop bcmwps
   if(bcmwps != NULL){

      //convert sta mac to string
      char curr_mac_sta[32];
      char curr_mac_ap[32];
      sprintf(curr_mac_sta, "%02x:%02x:%02x:%02x:%02x:%02x",
         bcmwps->mac_sta[0], bcmwps->mac_sta[1], bcmwps->mac_sta[2],
         bcmwps->mac_sta[3], bcmwps->mac_sta[4], bcmwps->mac_sta[5]);
      sprintf(curr_mac_ap, "%02x:%02x:%02x:%02x:%02x:%02x",
         bcmwps->mac_ap[0], bcmwps->mac_ap[1], bcmwps->mac_ap[2],
         bcmwps->mac_ap[3], bcmwps->mac_ap[4], bcmwps->mac_ap[5]);
      ...
      //get wps type
	char *dev_pwd = NULL;
	char curr_wps_type[16] = "N/A";
	strcpy(curr_wps_type,"client-pin");
	if (bcmwps->sc_mode == SCMODE_AP_ENROLLEE) {
		strcpy(curr_wps_type,"ap-pin");
	}else if (bcmwps->sc_mode == SCMODE_AP_REGISTRAR) {
		dev_pwd = wps_ui_get_env("wps_sta_pin");
		if (strncmp(dev_pwd, "00000000", 8) == 0)
			strcpy(curr_wps_type,"client-pbc");
	}	
      ...
      //4: ConnectionFailed 
      //5: GettingIP
      char curr_wps_state[16];
      if(state == 1){
         strcpy(curr_wps_state,"Associating");
      }else if(state == 2){
         strcpy(curr_wps_state,"Connected");
      }else if(state == 3){
         strcpy(curr_wps_state,"IncorrectPin");
#if 0 //__Qwest__, Ling-Chun
		 if((wps_initCmsMsg()) == 0){
		    wps_sendUpdateConfErrCmsMsg();  
		    wps_cleanCmsMsg();
		 }else
         ...
         strcpy(curr_wps_state,"ConnectFailed");
#if 0 //__Qwest__, Ling-Chun
		 if((wps_initCmsMsg()) == 0){
		    wps_sendUpdateConfErrCmsMsg();  
		    wps_cleanCmsMsg();
         ...
         strcpy(curr_wps_state,"GettingIP");
      }else{
         strcpy(curr_wps_state,"N/A");
      }

      //1. scan the old record and copy it to new file
      //2. put the current wps record to file
      int found = 0;
      int found_ap_pin = 0;
      char line[128];
      char wps_mac[32] = "";
      ...
      char wps_state[16] = ""; 
      char wps_time[16] = "";
      while ( fp_read!= NULL && fgets ( line, sizeof(line), fp_read ) != NULL ){
         sscanf(line,"%s %s %s %s %s %s",wps_mac,wps_name,wps_ip,wps_type,wps_state,wps_time);

         if(!strcmp(wps_type,"ap-pin")){
            found_ap_pin = 1;
         }

         
         if(!strcmp(wps_mac,curr_mac_sta)){
            found = 1;
            break;
         }

         fprintf(fp_write,"%s %s %s %s %s %s\n",wps_mac,wps_name,wps_ip,wps_type,wps_state,wps_time);
      }

      if(found == 1){
         if(strcmp(wps_state,curr_wps_state)){
            sprintf(wps_time,"%ld", time(0));
         }
         
         sprintf(wps_type,"%s",curr_wps_type);
         sprintf(wps_state,"%s",curr_wps_state);
         //befro Connected, the wps name and wps ip should be empty
         sprintf(wps_name,"N/A");
         sprintf(wps_ip,"N/A");
         ...
         fclose(fp_write);
         remove("/var/wps");
         rename("/var/wps.tmp","/var/wps");
         
      }else if(found == 0){
         
         if(found_ap_pin == 1 && !strcmp(wps_state,"Associating") && !strcmp(curr_wps_state,"Associating")){
            //only one ap_pin associating is allow 
            //do nothing
         }else{
            strcpy(wps_mac,curr_mac_sta);
            sprintf(wps_name,"N/A");
            sprintf(wps_ip,"N/A");
            sprintf(wps_type,"%s",curr_wps_type);
            sprintf(wps_state,"%s",curr_wps_state);
         ...
         //No need to write rest records
         //No need to rewind the file
         if(fp_read!= NULL){
            fclose(fp_read);
            remove("/var/wps");
         }
...
//**************************************************************************
// Function Name: wpsap_exit_wps_list
// Description  : Once wps is exit process message, it should do something to make things normal
// Parameters   : None
// Returns      : None.
//**************************************************************************
void wpsap_exit_wps_list(int retVal,wpsap_wksp_t *wps_wksp){
   //open a wps.tmp for temporary write.
   FILE *fp_write = fopen("/var/wps.tmp", "w");
   if(fp_write == NULL){
      return;   
   ...
   while ( fp_read!= NULL && fgets ( line, sizeof(line), fp_read ) != NULL ){
      ...
      if(!strcmp(wps_state,"Associating")){
         sprintf(wps_time,"%ld", time(0));
         strcpy(wps_state,"ConnectFailed");
#if 1 //__Qwest__, Ling-Chun	
		 int val = wps_getProcessStates();
		 if(val!=WPS_M2D)	
			wps_setProcessStates(WPS_TIMEOUT);	
#endif
      }else if(!strcmp(wps_state,"GettingIP")){
         sprintf(wps_time,"%ld", time(0));
         strcpy(wps_state,"ConnectFailed");
      }
...
//**************************************************************************
// Function Name: wpsap_check_close_session
// Description  : Remove all zero mac. In some ap-pin case, there would all zero mac. Remove it after wps process is completed
// Parameters   : None
// Returns      : None.
//**************************************************************************
void wpsap_check_close_session(){

   //read the config file
   FILE *fp_conf = fopen("/var/wirelessstatus", "r");
   if( fp_conf == NULL ){
      return 0;
   }
   char curr_wps_type[16] = "N/A";
   ...
   char conf_value[16];
   char conf_line[128];
   while(fp_conf != NULL && fgets ( conf_line, sizeof(conf_line), fp_conf ) != NULL){
      //wpsstatus=client-pbc
      sscanf(conf_line,"%[^'=']=%s",conf_key,conf_value);
      if(!strcmp(conf_key,"wpsstatus")){
         strcpy(curr_wps_type,conf_value);
         break;
      }
   }
   fclose(fp_conf);

   if(!strcmp(curr_wps_type,"ap-pin")){
      //open a wps.tmp for temporary write.
      FILE *fp_write = fopen("/var/wps.tmp", "w");
      if(fp_write == NULL){
         return;   
      ...
      while ( fp_read!= NULL && fgets ( line, sizeof(line), fp_read ) != NULL ){
         sscanf(line,"%s %s %s %s %s %s",wps_mac,wps_name,wps_ip,wps_type,wps_state,wps_time);
         if(!strcmp(wps_mac,"00:00:00:00:00:00")&&!strcmp(wps_type,"ap-pin")){
            continue;
         }
