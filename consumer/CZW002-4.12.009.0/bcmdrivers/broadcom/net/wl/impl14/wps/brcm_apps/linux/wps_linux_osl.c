 ...
 *
 * $Id: wps_linux_osl.c 280299 2011-08-29 04:01:31Z $
 */
...
#ifdef WLAN_CENTRYLINK //__Qwest__, SinJia
...
#ifdef WLAN_CENTRYLINK //__CTLK__, LingChun, __Qwest__, SinJia
//**************************************************************************
// Function Name: wps_setFileVar
// Description  : Copy from wlutil.c. Set parameter from /var/wirelessstatus.
// Parameters   : None
// Returns      : None.
//**************************************************************************
void wps_setFileVar(char* varName, char* varValue){
	char size[2048]={0};	
    char token[1024]={0};
	char* cp = NULL;
    int isFound=0;	
	...
	FILE *fp = fopen("/var/wirelessstatus", "r");	   
	if(fp!=NULL){	
		while(fscanf(fp,"%s ",token)==1){
         cp = strchr(token,'=');  
			if(cp) {
              *cp='\0';
              if(!strcmp(varName,token)){ //use newValue
                   sprintf(size,"%s%s=%s\n",size,varName,varValue); 
...
void wps_setFileVarInt(char* varName, int varValue){
   char val[8] = "";
   sprintf(val, "%d", varValue);
   wps_setFileVar(varName, val);
}


//**************************************************************************
// Function Name: wps_getFileVar
// Description  : Copy from wlutil.c. Get parameter from /var/wirelessstatus.
//                     If the varName could not found, it would return empty string.
// Parameters   : None
// Returns      : None.
//**************************************************************************
void wps_getFileVar(char* varName, char* varValue){
   strcpy(varValue, "");
	...
	if(fp!=NULL){
		while(fscanf(fp,"%s\n",token)==1){
		  ...
		  if(cp) {
			  *cp='\0';   
			  if(!strcmp(varName,token)){ //found		  
				sprintf(varValue,"%s",cp + 1);					  
			  } 	   
		  } 			   
		  memset(token,0,sizeof(token));
		}
...
//**************************************************************************
// Function Name: wps_initCmsMsg
// Description  : Init the cms handler to send. Only call once for a wps process. Call the 
//                     function when wps_ap is init.
// Parameters   : None
// Returns      : None.
//**************************************************************************
int wps_initCmsMsg(){
   ...
   {
      printf("%s@%d could not initialize msg, ret=%d", __FUNCTION__,__LINE__,ret);
      return (int)ret;
   }
...
//**************************************************************************
// Function Name: wps_sendUpdateConfErrCmsMsg
// Description  : Send  the config error message to ssk
// Parameters   : None
// Returns      : None.
//**************************************************************************
void wps_sendUpdateConfErrCmsMsg(){

   char buf[sizeof(CmsMsgHeader) + 32]={0};
   CmsMsgHeader *msg=(CmsMsgHeader *) buf;
   CmsRet ret = CMSRET_SUCCESS;

#if 1 //__Qwest__, Ling-Chun, wldaemon will update wps execution status
   sprintf((char *)(msg + 1), "Update:%d", 1);
#endif

   msg->type = CMS_MSG_WPS_UPDATE_MSG;
   msg->src = EID_WLWPS;
   msg->dst = EID_WLMNGR;
   ...
   if ((ret = cmsMsg_send(msgHandle, msg)) != CMSRET_SUCCESS){
      printf("could not send CMS_MSG_WLAN_CHANGED msg to ssk, ret=%d", ret);
   }else{
...
//**************************************************************************
// Function Name: wps_cleanCmsMsg
// Description  : Free the msg handler
// Parameters   : None
// Returns      : None.
//**************************************************************************
void wps_cleanCmsMsg(){
   if ( msgHandle )cmsMsg_cleanup(&msgHandle);
}
#endif
