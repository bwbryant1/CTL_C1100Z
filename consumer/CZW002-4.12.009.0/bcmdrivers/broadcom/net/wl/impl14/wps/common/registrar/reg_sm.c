 ...
 *
 * $Id: reg_sm.c 323637 2012-03-26 10:31:40Z $
 */
...
reg_sm_handleMessage(RegSM *r, BufferObj *msg, BufferObj *outmsg)
{
	...
	case MSTART:
      ...
      if(reg_proto_testDisplayCfg() == 0){
         retVal = state_machine_send_nack(r->reg_info, WPS_MESSAGE_PROCESSING_ERROR, outmsg, NULL);

         TUTRACE((TUTRACE_INFO, "RPROTO: Reject by Display restricted\n"));
         
         goto out;
      }

      if(reg_proto_testPushButtonCfg()== 0){
         retVal = state_machine_send_nack(r->reg_info, WPS_MESSAGE_PROCESSING_ERROR, outmsg, NULL);

         TUTRACE((TUTRACE_INFO, "RPROTO: Reject by PushButton restricted\n"));
...
int reg_proto_testDisplayCfg(){
   int valid = 1;
   char varConfig[32], varMethod[32];
   
   wps_getFileVar("wpsDisplayCfg", varConfig);
   wps_getFileVar("wpsstatus", varMethod);

   if(!strcmp(varMethod, "client-pin") && !strcmp(varConfig, "0")){
      valid = 0;
   }else{
      valid = 1;
   }
...
int reg_proto_testPushButtonCfg(){
   ...
   wps_getFileVar("wpsPushButtonCfg", varConfig);
   wps_getFileVar("wpsstatus", varMethod);

   if(!strcmp(varMethod, "client-pbc") && !strcmp(varConfig, "0")){
      valid = 0;
   }else{
      valid = 1;
   }
