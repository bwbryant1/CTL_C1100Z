 ...
 *
 * $Id: wps_ie.c 291537 2011-10-24 07:30:36Z $
 */
...
wps_set_prb_rsp_IE(int ess_id, char *ifname, CTlvSsrIE *ssrmsg)
{
   ...
   if(wps_ie_testWpsLockState() == 0){ 
      TUTRACE((TUTRACE_INFO, "AP in lock up state by wps_ie_testWpsLockState(), deny AP configuring\n"));
      data8 = 0x1;
		tlv_serialize(WPS_ID_AP_SETUP_LOCKED, bufObj, &data8, WPS_ID_AP_SETUP_LOCKED_S);
   }
#endif
