 ...
 *
 * $Id: wps_ui.c 381832 2013-01-30 02:35:37Z $
 */
...
wps_ui_start_pending(char *wps_ifname)
{
	...
	if (b_do_pbc && wps_pb_check_pushtime(now) == PBC_OVERLAP_CNT) {
		...
		wps_gpio_led_blink(WPS_BLINKTYPE_OVERLAP);
#endif

#if 1 //__Qwest__, SinJia
		  //Detect pbc overlap periodically when overlap is happened
		  pid_t child;
		  switch (child = fork()) {
			 case -1: /* error */
				break;
			 ...
			 case 0:  //child
				while(1){
				   ...
				   if(wps_getProcessStates()==WPS_PBCOVERLAP){
					  (void) time((time_t*)&now);
					  if (wps_pb_check_pushtime(now) == PBC_OVERLAP_CNT) {
						 continue;
					  }else{
						 wps_setProcessStates(WPS_INIT);
						 
						 //set process state only would not trigger led set (?)
						 //so set the led again
						 wps_gpio_led_blink(WPS_BLINKTYPE_ON);
						 break;
					  }
				   }else{
					  break;
...
wps_ui_do_set(char *buf)
{
	...
	if (ui->wps_config_command == WPS_UI_CMD_STOP) {
		...
		int testExpire = wps_ui_pending_expire();

		/* state maching in progress, check stop or push button here */
		wps_close_session();

		if(testExpire){
			 //set to blink timeout leds
