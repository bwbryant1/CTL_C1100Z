 ...
 *
 * $Id: enr_reg_sm.c 321622 2012-03-16 04:10:38Z $
 */
...
enr_sm_initializeSM(EnrSM *e, DevInfo *dev_info)
{
	...
	if (memcmp(dev_info->pre_nonce, empty_enrolleeNonce, SIZE_128_BITS)) {
	...
	case M1:
		...
		/* message wasn't M2D, do processing for M2 */
			...
			if (WPS_SUCCESS != err) {
               ...
               configError = WPS_CONFIG_ERROR_DEVICE_PASSWORD_AUTH_FAILURE;
#endif
					goto out;
				}
...
#ifdef MTS_WLAN_WPS_FOR_TR69 //__Qwest__, SinJia, TR069 for wps
               configError = WPS_CONFIG_ERROR_DEVICE_BUSY;
#endif
					goto out;
				} /* WSC 2.0 */
...
#ifdef MTS_WLAN_WPS_FOR_TR69 //__CTLK__, LingChun
				configError = WPS_CONFIG_ERROR_ROUGE_ACTIVITY_SUSPECTED;
#endif
					goto out;
				}
...
#ifdef MTS_WLAN_WPS_FOR_TR69 //__Qwest__, SinJia, TR069 for wps
               configError = WPS_CONFIG_ERROR_DECRYPTION_CRC_FAILURE;
#endif
					/* send msg_to_send message immedicate */
					enr_sm_restartSM(e);
					goto out;
...
#ifdef MTS_WLAN_WPS_FOR_TR69 //__CTLK__, LingChun, TR069 for wps
            configError = WPS_CONFIG_ERROR_DEVICE_BUSY;
#endif
			}
			enr_sm_restartSM(e);
...
#ifdef WLAN_CENTRYLINK //__CTLK__, LingChun
		if (regInfo->enrollee->b_ap && strcmp(wps_safe_get_conf("wps_enr_block_settings"), "1") == 0) {			
			TUTRACE((TUTRACE_ERR, "ENRSM: ENR block the settings = enable %x\n", err));
			retVal = state_machine_send_nack(e->reg_info, WPS_ERROR_FAIL_CONN_REGISTRAR, outmsg, NULL);
			retVal = WPS_MESSAGE_PROCESSING_ERROR;
			goto out;
		}else
			TUTRACE((TUTRACE_ERR, "ENRSM: ENR block the settings = disable. \n"));
#endif
...
#ifdef MTS_WLAN_WPS_FOR_TR69 //__Qwest__, SinJia, TR069 for wps
   wps_setFileVar("wpsEnrUuid", m.uuid.m_data);
   wps_setFileVar("wpsEnrName", m.deviceName.m_data);      
#endif 
