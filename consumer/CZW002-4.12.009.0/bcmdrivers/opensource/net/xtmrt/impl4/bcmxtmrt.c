                                 ...
                                 FkBuff_t *pFkb, UINT16 bufStatus, int delLen, int trailerDelLen )
{
...
#else
   {
     ...
     else
     {
        UINT8 isAtm = 0;
        UINT8 isTagged = 0;
        UINT8 VLAN_TYPE_DATA[2] = {0x81, 0x00};
        ...
        if (matchedService != 0)
        {
           ...
           if (isAtm)
           {
              ...
              printk("autoHunt:%s:%d: matchedService=%lu, vpi=%u, vci=%u\n",
                 __func__, __LINE__, matchedService, pDevCtx->Addr.u.Vcc.usVpi, pDevCtx->Addr.u.Vcc.usVci);

           }
           else
           {
              if (pDevCtx->ucHuntedVidCount < HUNT_MAX_VID)
              {
                 if (isTagged)
                 {
                    pDevCtx->huntedVidInfo[pDevCtx->ucHuntedVidCount].lHuntedVid = FindHuntVlanId_MSTC(skb);
                 }
                 else
                 {
                    pDevCtx->huntedVidInfo[pDevCtx->ucHuntedVidCount].lHuntedVid = -1;
                 }
                 pDevCtx->huntedVidInfo[pDevCtx->ucHuntedVidCount].ulHuntedVidService = matchedService; 

                 printk("autoHunt:%s:%d: matchedService=%lu, vlanId=%ld\n",
                    __func__, __LINE__, matchedService, pDevCtx->huntedVidInfo[pDevCtx->ucHuntedVidCount].lHuntedVid);

                 pDevCtx->ucHuntedVidCount++;
              }
              else
              {
                 printk("%s, %d, pDevCtx->ucHuntedVidCount is MAX\n", __func__, __LINE__);
              }
           }
        }
        kfree_skb(skb);
...
static UINT32 FindHuntServiceType_MSTC(struct sk_buff *skb, UINT8 isTagged)
{
   ...
   else if (memcmp(CFG_ACK_PPPoA, (dp), 2) == 0) 
   {
      /* match CONFIG ACK for PPPoA ?*/
   ...
   else if ((memcmp(ARP_TYPE, (dp+12+shift), 2) == 0) && (memcmp(ARP_REPLY, (dp+20+shift), 2) == 0)) 
   {
      ...
      if (memcmp(&pGi->ulHuntGatewayIp, (dp+28+shift), 4) == 0)
      {
...
static int DoCreateDeviceReq( PXTMRT_CREATE_NETWORK_DEVICE pCnd )
{
         ...
         pCnd->szNetworkDeviceName, ether_setup )) != NULL )
    {
        ...
        kerSysGetMacAddress(dev->dev_addr, macId);
#endif // #if 1 // __CTLK__,JhenYang, refer to common 412
...
static int DoSendVCHuntTestPattern(PBCMXTMRT_DEV_CONTEXT pDevCtx, PXTMRT_VCHUNT_TEST pParm)
{
   ...
   if (pDevCtx == NULL)
   {
      printk("autoHunt:%s:%d: pDevCtx is NULL\n", __func__, __LINE__);
      return (-EPERM);
   }
         ...
         && (pDevCtx->ulHdrType == pParm->ulTestHeaderType))
   {
      ...
      while (sendType != 0)
      {
               ...
               ||HT_TYPE_VC_MUX_ETHERNET == HT_TYPE(pParm->ulTestHeaderType))
         {
            ...
            if(pParm->debugMode)
#endif
            printk("autoHunt:%s:%d: device vpi=%u, vci=%u\n",
               __func__, __LINE__, pDevCtx->Addr.u.Vcc.usVpi, pDevCtx->Addr.u.Vcc.usVci);
            ...
            else if (sendType & VC_HUNT_SERV_OPT_PPPOE_PADT)
            {
               ...
               memcpy((unsigned char *) &baseMacAddr, &pDevCtx->pDev->dev_addr[3], 3); 
               baseMacAddr >>= 8;			   
               baseMacAddr = (baseMacAddr + 2) << 8;
               memcpy( modifyAddr, pDevCtx->pDev->dev_addr, 6);
               ...
               memcpy(dataTmp, pParm->ulSession.MACAddress, 6);
               memcpy(dataTmp+16, pParm->ulSession.SessionID, 2);
               memcpy(dataTmp+24, pParm->ulSession.HostID, 4);
            ...
            else if (sendType & VC_HUNT_SERV_OPT_STATICIP)
            {
               ...
               if(pParm->debugMode)
#endif
               printk("autoHunt:%s:%d: send Stati IP pattern (ARP)\n", __func__, __LINE__);
               ...
               pParm->ulPingSrcIp,
               NULL,
               pDevCtx->pDev->dev_addr,
               NULL);

               arp_skb->dev = pDevCtx->pDev;

               if (arp_skb)
               {
                  if (pDevCtx->ulVcHuntStatus == VC_HUNT_IDLE)
                  {
                     pDevCtx->ulVcHuntStatus = VC_HUNT_TRYING;
                  }
                  bcmxtmrt_xmit(SKBUFF_2_PNBUFF(arp_skb), pDevCtx->pDev);
               }

               /* save Static Info */
               pGi->ulHuntSrcIp = pParm->ulPingSrcIp;
               pGi->ulHuntGatewayIp = pParm->ulPingGatewayIp;
               ...
               || HT_TYPE_VC_MUX_PPPOA == HT_TYPE(pParm->ulTestHeaderType))
         {
            ...
            if( sendType & VC_HUNT_SERV_OPT_PPPOA)
            {
               ...
               if(pParm->debugMode)
#endif
               printk("autoHunt:%s:%d: send PPPoA pattern\n", __func__, __LINE__);
         ...
         else if (HT_TYPE_PTM == HT_TYPE(pParm->ulTestHeaderType))
         {
#if 1 //__CTLK__, SeanLu 20150306, Support vlanautohunt DebugMode			
            if(pParm->debugMode)
#endif
            printk("autoHunt:%s:%d: device vlanId=%ld\n",
               __func__, __LINE__, pParm->lTestVlanId);
            ...
            if (sendType & VC_HUNT_SERV_OPT_PPPOE)
            {
               ...
               if (pParm->lTestVlanId != -1)
               {
                  ...
                  dataTmp[12] = 0x81;
                  dataTmp[13] = 0x00;
                  dataTmp[14] = (pParm->lTestVlanId/256);
                  dataTmp[15] = (pParm->lTestVlanId%256);
            ...
            else if (sendType & VC_HUNT_SERV_OPT_STATICIP)
            {
               ...
               if (pParm->lTestVlanId != -1)
               {
                  arp_skb = vlan_put_tag(arp_skb, pParm->lTestVlanId);
               }
         ...
         if (skb)
         {
...
#if 1
            {
               ...
               ret_bcmrt = bcmxtmrt_xmit(SKBUFF_2_PNBUFF(skb), pDevCtx->pDev);
#if 1 //__CTLK__, SeanLu 20150306, Support vlanautohunt DebugMode						   
               if(pParm->debugMode)		
#endif
...
static int DoGetVcHuntTestStatusReq(PBCMXTMRT_DEV_CONTEXT pDevCtx, PXTMRT_VCHUNT_TEST_STATUS pParm)
{
   ...
   if (pDevCtx->ulLinkState == LINK_UP)
   {
      ...
      pParm->ucHuntedVidCount = pDevCtx->ucHuntedVidCount;
      memcpy(pParm->huntedVidInfo, pDevCtx->huntedVidInfo, HUNT_MAX_VID*sizeof(XTM_AUTOHUNT_VID_INFO));
   }
   else
   {
      printk("%s, %d, pDevCtx->ulLinkState failed \n" "\033[m", __func__, __LINE__);
      nRet = -EPERM;
   }
