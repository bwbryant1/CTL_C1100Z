...
#elif defined(ROBOSW250_CLK_EN)
#if 1
            /* software reboot fail once if CPE didn't patch svn r4846 bugfix. for old CFE version, P880 should not disable RS_PLL250_CLK_EN */
            //PERF->blkEnables &= ~ROBOSW250_CLK_EN;
#else
            PERF->blkEnables &= ~ROBOSW250_CLK_EN;
#endif
...
{
   uint16 v16 = 0x0024;
   extsw_wreg_wrap(PAGE_CONTROL, 0x12, (uint8 *)&v16, 2);
}
#endif
...
/*Configure port state for HPAN module*/
#if defined(BUILD_HPNA)
		//printk("\n\r %s@%d: configure hpna port %d\n\r", __FUNCTION__, __LINE__, port) ;
		if(port == 5){
			/*Enabled EEE function per port
				[5:1]=Port 1~5; [7:6]=Reserved*/
			v16 |= 0x0f ;
			extsw_wreg_wrap(PAGE_EEE, REG_EEE_EN_CTRL, (uint8 *)&v16, 2);
			/*Port State
				[0]=Link State; [1]=Duplex Mode; 
				[3:2]=Speed; 00=10M, 01=100M, 10=1G, 11: 200M
				[4]: Rx Flow Ctrl Enabled; [5]: Tx Flow Ctrl Enabled; 
				[6]: Override; 1=allows the value of the bits[5:0] 0 write to external PHY
				[7]: Reserved
				*/
			v16 = 0x7f ;
			extsw_wreg_wrap(PAGE_CONTROL, REG_CTRL_OVERRIDE+4, (uint8 *)&v16, 2);
		}
#endif
...
void bcmsw_port_mirror_set (int enable, int mirror_port, unsigned int ing_pmap, unsigned int eg_pmap, unsigned int blk_no_mrr)
{
    ...
    if (enable)
    {
        ...
        extsw_wreg_wrap(PAGE_MANAGEMENT, REG_MIRROR_CAPTURE_CTRL, (uint8 *)&v16, 2);
        v16 = ing_pmap & REG_INGRESS_MIRROR_M;
        extsw_wreg_wrap(PAGE_MANAGEMENT, REG_MIRROR_INGRESS_CTRL, (uint8 *)&v16, 2);
        v16 = eg_pmap & REG_INGRESS_MIRROR_M;
        extsw_wreg_wrap(PAGE_MANAGEMENT, REG_MIRROR_EGRESS_CTRL, (uint8 *)&v16, 2);
    }
    else
    {
        v16  = 0;
...
{
    uint16 v16;
    extsw_rreg_wrap(PAGE_MANAGEMENT, REG_MIRROR_CAPTURE_CTRL, (uint8 *)&v16, 2);
    if (v16 & REG_MIRROR_ENABLE)
    {
        *enable = 1;
        *mirror_port = v16 & REG_CAPTURE_PORT_M;
        *blk_no_mrr = v16 & REG_BLK_NOT_MIRROR;
        extsw_rreg_wrap(PAGE_MANAGEMENT, REG_MIRROR_INGRESS_CTRL, (uint8 *)&v16, 2);
        *ing_pmap = v16 & REG_INGRESS_MIRROR_M;
        extsw_rreg_wrap(PAGE_MANAGEMENT, REG_MIRROR_EGRESS_CTRL, (uint8 *)&v16, 2);
