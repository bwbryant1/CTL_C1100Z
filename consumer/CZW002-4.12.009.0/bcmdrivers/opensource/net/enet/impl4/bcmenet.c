...
#ifdef BUILD_HPNA
#define ETH_WAN_DEV_NAME "eth5"
#else
#define ETH_WAN_DEV_NAME "eth4"
#endif
#endif
...
#endif /* CONFIG_BCM96816 */
#if 1 //__MSTC__, JhihPing, /* auto hunt */
    UINT8 autoHuntWorking;   /* auto hunt enable */
    UINT32 ulHuntSrcIp;        /* hunt static IP */
    UINT32 ulHuntGatewayIp;    /* hunt static IP */
    UINT32 autoHuntStatus;
    UINT8 autoHuntVidCount;
    XTM_AUTOHUNT_VID_INFO autoHuntVidInfo[HUNT_MAX_VID];
#endif
...
#if 1 /*KuanJung Reverse vport <--> phyport mapping*/
		/* phyport  -> j:  port1| port2| port3| port4		   */
		/* vport      -> i:  eth4 | eth2 | eth3 | eth0 		   */
...
void link_change_handler(int port, int linkstatus, int speed, int duplex)
{
    ...
    int linkMask;
    int sw_port;
#if 1 // __CTLK__, SeanLu 20150630, Fix ethernet wan can't receive packets after vlanautohunt finish
    unsigned int msgData = 0;
#endif
...
#if 1 // __CTLK__, SeanLu 20150630, Fix ethernet wan can't receive packets after vlanautohunt finish
        if((strcmp(pNetDev->name, ETH_WAN_DEV_NAME) == 0))
        {
            if(gmac_info_pg->wan)
            {
                //printk((KERN_CRIT "LAN/WAN interface %s as WAN.\n"), pNetDev->name);
                msgData = 1;
                kerSysSendtoMonitorTask(MSG_NETLINK_BRCM_LINK_STATUS_CHANGED, (void *)&msgData, sizeof(msgData));
            }
            else
            {
                //printk((KERN_CRIT "LAN/WAN interface %s as LAN.\n"), pNetDev->name);
                msgData = 0;
...
#if 1 //__MSTC__, JhihPing, /* auto hunt */
static UINT32 FindHuntServiceType_MSTC(struct sk_buff *skb, UINT8 isTagged)
{
   UINT32 service = 0;
   UINT8 *dp = skb->data;
   UINT8 PADO_POE[6] = {0x88, 0x63, 0x11, 0x07, 0x0, 0x0};
   UINT8 DHCP_PORT_INFO[4] = {0x00, 0x43, 0x00, 0x44};	/* src = 67, dest = 68 */
   UINT8 ARP_TYPE[2] = {0x08, 0x06};
   UINT8 ARP_REPLY[2] = {0x00, 0x02};
   UINT8 shift = 0;

   /* vlan type + vlan ID = 4 bytes */
   if (isTagged)
   {
      shift = 4;
   }
   ...
   if (memcmp(PADO_POE, (dp+12+shift), 6) == 0) 
   {
      /* match PADO for PPPoE ?*/
      service = VC_HUNT_SERV_OPT_PPPOE;
   }
   else if (memcmp(DHCP_PORT_INFO, (dp+34+shift), 4) == 0)
   {
      /* DHCP Ack for Enet encap */
      service = VC_HUNT_SERV_OPT_ENETENCAP;
   }
   else if ((memcmp(ARP_TYPE, (dp+12+shift), 2) == 0) && (memcmp(ARP_REPLY, (dp+20+shift), 2) == 0)) 
   {
      if (memcmp(&global.ulHuntGatewayIp, (dp+28+shift), 4) == 0)
      {
         /* match ARP Reply for hunt on Static IP */
         service = VC_HUNT_SERV_OPT_STATICIP;
      }
   }
   else
   {
...
static INT32 FindHuntVlanId_MSTC(struct sk_buff *skb)
{
   INT32 vlanId = 0;
   UINT8 *dp = skb->data;

   vlanId = (dp[14] & 0x0F)*256 + dp[15];
...
#endif
#if 1 //__MSTC__, JhihPing, /* auto hunt */
   unsigned long matchedService = 0; 
#endif
...
#if 1 //__MSTC__, JhihPing, /* auto hunt */
                if (global.autoHuntWorking && (strcmp(dev->name, ETH_WAN_DEV_NAME) == 0))
                {
                   UINT8 VLAN_TYPE_DATA[] = {0x81, 0x00};
                   UINT8 isTagged = 0;

                   if (memcmp(skb->data+12, VLAN_TYPE_DATA, 2) == 0)
                   {
                      isTagged = 1;
                   }
                   else
                   ...
                   matchedService = FindHuntServiceType_MSTC(skb, isTagged);
                   if (matchedService != 0)
                   {
                      global.autoHuntStatus = VC_HUNT_RECVRESP;

                      if (global.autoHuntVidCount < HUNT_MAX_VID)
                      {
                         if (isTagged)
                         {
                            global.autoHuntVidInfo[global.autoHuntVidCount].lHuntedVid = FindHuntVlanId_MSTC(skb);
                         }
                         else
                         {
                            global.autoHuntVidInfo[global.autoHuntVidCount].lHuntedVid = -1;
                         }
                         global.autoHuntVidInfo[global.autoHuntVidCount].ulHuntedVidService = matchedService; 

                         //printk("autoHunt:%s:%d: matchedService=%lu, vlanId=%ld\n",
                         //   __func__, __LINE__, matchedService, 
                         //   global.autoHuntVidInfo[global.autoHuntVidCount].lHuntedVid);

                         global.autoHuntVidCount++;
                      }
                      else
                      {
                         printk("autoHunt:%s:%d: hunted Vid count is MAX\n", __func__, __LINE__);
                      }
                   }
                   goto next_rx;
                }
...
#if 1 //config the bcm53124 LED function for DSL-2492GNUP-B1DC. refer to CSP 564434
		extsw_led_init();
#endif
    }
#endif
...
#if 1 // __CLTK__, SeanLu 20150123, Support Send PADT
const static UINT8 PADT_PPPOE [] ={
	0x00, 0x19, 0xcb, 0xcb, 0xcb, 0xcb, 0x00, 0x13, 0x49, 0x01, 0xd2, 0x3e, 0x88, 0x63, 0x11, 0xa7, 
	0x00, 0x09, 0x00, 0x0c, 0x01, 0x03, 0x00, 0x04, 0x00, 0x48, 0xe0, 0x18, 0x01, 0x01, 0x00, 0x00,
};
#endif
...
static int enet_ioctl_send_hunt_pattern(struct ethswctl_data *e)
{
   PXTM_VCHUNT_TEST_CFG pParm = &e->enetVt;
   struct net_device *dev = NULL;
   UINT32 sendType = 0;
   UINT8 found = 0;
   int nRet = 0;
   ...
   for (i = 0; i < TOTAL_SWITCH_PORTS - 1; i++) 
   {
      ...
      if (dev && (strcmp(dev->name, ETH_WAN_DEV_NAME) == 0))
      {
         found = 1;
         break;
      }
   } 
#if 1 //__CTLK__, SeanLu 20150306, Support vlanautohunt DebugMode
   if(pParm->debugMode)
#endif
   printk("autoHunt:%s:%d: send vlanId=%ld\n", __func__, __LINE__, pParm->lVlanId);

   sendType = pParm->ulServiceTypeForHunting;
   ...
   if (found)
   {
      ...
      if ((dataTmp = kmalloc(512, GFP_ATOMIC)) == NULL)
      {
         //printk("autoHunt:%s:%d: kmalloc error\n", __func__, __LINE__);
         return -ENOMEM;
      }
      ...
      while (sendType != 0)
      {
         ...
         if (sendType & VC_HUNT_SERV_OPT_PPPOE)
         {
            ...
            if(pParm->debugMode)
#endif
            printk("autoHunt:%s:%d: send PPPoE pattern\n", __func__, __LINE__);
            ...
            memcpy(dataTmp+6, dev->dev_addr, 6);

            if (pParm->lVlanId != -1)
            {
               /* insert 4 bytes of vlan info, [12][13]: vlan type, [14][15]: vlan id */
               UINT32 i = 0;
               UINT32 dataTmpLen = sizeof(PADI_PPPOE);

               /* shift data */
               for (i = dataTmpLen; i >= 12; i--)
               {
                  dataTmp[i+4] = dataTmp[i];
               }
               dataTmp[12] = 0x81;
               dataTmp[13] = 0x00;
               dataTmp[14] = (pParm->lVlanId/256);
               dataTmp[15] = (pParm->lVlanId%256);

               hrdPayloadSize = sizeof(PADI_PPPOE) + 4;   /* add vlan 4 bytes */
            }
            else
            {
...
#if 1 // __CLTK__, SeanLu 20150123, Support Send PADT
         else if (sendType & VC_HUNT_SERV_OPT_PPPOE_PADT)
         {
            unsigned long baseMacAddr = 0;
            unsigned char modifyAddr[6];

#if 1 //__CTLK__, SeanLu 20150306, Support vlanautohunt DebugMode			
            if(pParm->debugMode)
#endif
            printk("autoHunt:%s:%d: send PPPoE PADT pattern\n", __func__, __LINE__);	
            headTmp = (UINT8*)&PADI_PPPOE_HEAD; 
            newheadroom = sizeof(PADI_PPPOE_HEAD);
            memset(dataTmp, 0, 512*sizeof(UINT8));
            memcpy(dataTmp, PADT_PPPOE, sizeof(PADT_PPPOE));
			
            /* Modify the src MAC with base MAC +2 as WAN mac */
            memcpy((unsigned char *) &baseMacAddr, &dev->dev_addr[3], 3); 
            baseMacAddr >>= 8;			   
            baseMacAddr = (baseMacAddr + 2) << 8;
            memcpy( modifyAddr, dev->dev_addr, 6);
            memcpy( modifyAddr + 3, (unsigned char *) &baseMacAddr, 3 );
            memcpy( dataTmp+6, modifyAddr, 6);
			
            /* Copy the old session dest MAC address, old session ID and Host-uniq Value */			
            memcpy(dataTmp, pParm->ulSessionInfo.MACAddress, 6);
            memcpy(dataTmp+16, pParm->ulSessionInfo.SessionID, 2);
            memcpy(dataTmp+24, pParm->ulSessionInfo.HostID, 4);
			
            if (pParm->lVlanId != -1)
            {
               /* insert 4 bytes of vlan info, [12][13]: vlan type, [14][15]: vlan id */
               UINT32 i = 0;
               UINT32 dataTmpLen = sizeof(PADT_PPPOE);
               ...
               hrdPayloadSize = sizeof(PADT_PPPOE) + 4;   /* add vlan 4 bytes */
            }
            else
            {
               hrdPayloadSize = sizeof(PADT_PPPOE);
            }
            ...
            TmpTc_verd = 0x2000 ;
            TmpMark = 0;
            sendType &= ~VC_HUNT_SERV_OPT_PPPOE_PADT ;
         }
#endif
         else if (sendType & VC_HUNT_SERV_OPT_ENETENCAP)
         {
            ...
            if(pParm->debugMode)
#endif
            printk("autoHunt:%s:%d: send DHCP pattern\n", __func__, __LINE__);
            ...
            if (pParm->lVlanId != -1)
            {
               /* insert 4 bytes of vlan info, [12][13]: vlan type, [14][15]: vlan id */
               UINT32 i = 0;
               UINT32 dataTmpLen = sizeof(DHCP_REQ_ENETENCAP);
               ...
               dhcp = dataTmp; 
               dhcp += 46;
               hrdPayloadSize = sizeof(DHCP_REQ_ENETENCAP) + 4;   /* add vlan 4 bytes */
            }
            else
            {
               udph = dataTmp; 
         ...
         else if (sendType & VC_HUNT_SERV_OPT_STATICIP)
         {
            struct sk_buff *arp_skb;
#if 1 //__CTLK__, SeanLu 20150306, Support vlanautohunt DebugMode			
            if(pParm->debugMode)
#endif
            printk("autoHunt:%s:%d: send Static IP pattern (ARP)\n", __func__, __LINE__);

            arp_skb = arp_create(ARPOP_REQUEST, ETH_P_ARP,
                  pParm->ulPingGatewayIp,
                  dev,
                  pParm->ulPingSrcIp,
                  NULL,
                  dev->dev_addr,
            ...
            if (pParm->lVlanId != -1)
            {
               arp_skb = vlan_put_tag(arp_skb, pParm->lVlanId);
            }

            if (arp_skb)
            {
               if (global.autoHuntStatus == VC_HUNT_IDLE)
               {
                  global.autoHuntStatus = VC_HUNT_TRYING;
               }
               bcm63xx_enet_xmit( SKBUFF_2_PNBUFF(arp_skb), dev);
            }

            /* save Static Info */
            global.ulHuntSrcIp = pParm->ulPingSrcIp;
            global.ulHuntGatewayIp = pParm->ulPingGatewayIp;

            sendType &= ~VC_HUNT_SERV_OPT_STATICIP;
            continue;
         }
         else
         {
         ...
         if (skb)
         {
            ...
            ret_bcmrt = bcm63xx_enet_xmit(SKBUFF_2_PNBUFF(skb), dev);
#if 1 //__CTLK__, SeanLu 20150306, Support vlanautohunt DebugMode						
            if(pParm->debugMode)
#endif
            printk("autoHunt:%s:%d: send packets result=%d\n\n", __func__, __LINE__, ret_bcmrt);
         }
         else
         {
            printk("autoHunt:%s:%d:skb is NULL\n", __func__, __LINE__);
            nRet = -ENOMEM;
            break;
         }
      } /* while sendType */
   ...
   {
      printk("autoHunt:%s:%d: can't find the device\n", __func__, __LINE__);
      nRet = -EPERM;
   }
...
static int enet_ioctl_get_hunt_status(struct ethswctl_data *e)
{
   int nRet = 0;

   if (copy_to_user((void*)(&e->enetVt.ulVcHuntStatus), (void*)&global.autoHuntStatus, sizeof(UINT32)))
   {
      return -EFAULT;
   }

   if (copy_to_user((void*)(&e->enetVt.ucHuntedVidCount), (void*)&global.autoHuntVidCount, sizeof(UINT8)))
   {
      return -EFAULT;
   }

   if (copy_to_user((void*)(&e->enetVt.huntedVidInfo), (void*)&global.autoHuntVidInfo, HUNT_MAX_VID*sizeof(XTM_AUTOHUNT_VID_INFO)))
   {
      return -EFAULT;
   }

   global.autoHuntStatus = VC_HUNT_IDLE;
   global.autoHuntVidCount = 0;
   memset(&global.autoHuntVidInfo, 0, HUNT_MAX_VID*sizeof(XTM_AUTOHUNT_VID_INFO));

   return nRet;
}

static int enet_ioctl_set_hunt_info(struct ethswctl_data *e)
{
   PXTM_VCAUTOHUNT_INFO pParm = &e->enetVi;
   int nRet = 0;

   global.autoHuntWorking = pParm->ucVcAutoHuntWorking;

   if (copy_to_user((void*)(&e->enetVi.ucIsVcAutoHuntWorking), (void*)&global.autoHuntWorking, sizeof(UINT8)))
   {
      return -EFAULT;
   }
...
static int bcm63xx_enet_ioctl(struct net_device *dev, struct ifreq *rq, int cmd)
{
    ...
    struct net_device_stats *vstats;
#ifdef SEPARATE_MAC_FOR_WAN_INTERFACES //__CTLK__,JhenYang, merge from common 412
    struct sockaddr sockaddr;
#endif
    int swPort;
    int phyId;
#if 0 /*__MSTC__, FuChia*/
    int isExtSW = 0;
#endif
...
/*__MTSC__, Delon Yu, Support HPNA, merge from 4.02L03 KJ*/
#ifdef BUILD_HPNA 
		case SIOCSENABLEPORT:       /* Read MII PHY register. */
			mii = (struct mii_ioctl_data *)&rq->ifr_data;
			ethsw_switch_manage_port_power_mode( *data /*port 0*/, 1);
			break;

		case SIOCSDISABLEPORT:       /* Read MII PHY register. */
			mii = (struct mii_ioctl_data *)&rq->ifr_data;
			ethsw_switch_manage_port_power_mode( *data /*port 0*/, 0);
			break;
#endif
...
#if 1 //__CTLK__, Kaiping, Ether LAN statistics, add dump external switch mib from 4.16L03
                    if (e->unit) {
                        val = ethsw_dump_mib_ext(e->port, e->type);
                    }
                    else
                    {
...
#endif
                    BCM_ENET_DEBUG("ethswctl ETHSWGETDUMPMIB ioctl");

                    if (e->unit) {
                        val = ethsw_get_mib_ext(e->port, e);
                    }
                    else
                    {
                        val = ethsw_get_mib(e->port, e);
                    }
                    break;
#endif
...
#if 1 //__MSTC__, JhihPing
                case ETHSWSENDHUNTPATTERN:
                    val = enet_ioctl_send_hunt_pattern(e);
                    break;
                case ETHSWGETHUNTSTATUS:
                    val = enet_ioctl_get_hunt_status(e);
                    break;
                case ETHSWSETHUNTINFO:
                    val = enet_ioctl_set_hunt_info(e);
                    break;
#endif
