...
extern int enet_logport_to_phyid(int log_port);
extern int enet_set_port_ctrl( int port, uint32_t val32 );
extern void enet_restart_autoneg(int log_port);

#if 1 // __CTLK__, SeanLu 20150625, Fix LAN/WAN can't work after CPE translates ETH WAN to PTM/ATM , merge from BRCM 416
...
int gmac_is_gmac_supported( void )
{
    ...
    if (BpGetDeviceOptions(&options) == BP_SUCCESS) {
        ...
        if (options & BP_DEVICE_OPTION_ENABLE_GMAC)
        {
            ...
            if (gmac_info_pg->chip_id == 0x6818)
#endif
                rc = 1;
        }
    }
...
void gmac_set_wan_port( int add )
{
    ...
    /* set default values */
#if 0 // __CTLK__, SeanLu 20150625, Fix LAN/WAN can't work after CPE translates ETH WAN to PTM/ATM , merge from BRCM 416
    ...
    switch( gmac_info_pg->mode )
    {
        ...
        case GMAC_MODE_LINK_SPEED: /* active port selected based on speed */
        {
            ...
            if (gmac_info_pg->active)
            {
                ...
                {
                    /* Disable ROBO RX & TX */
                    enet_set_port_ctrl( GMAC_PORT_ID, 3 );
                }
                else
                {
                    gmac_set_inactive();
...
#elif defined(CONFIG_BCM96818)
static void gmac_init_6818( void )
{
    /* Set bits [4:0] to 0xD to achieve 1G over iuDMA   */
    MISC->miscPllCtrl = (MISC->miscPllCtrl & 0xFFFFFFE0) | 0x0000000D;

    BCM_LOG_DEBUG( BCM_LOG_ID_GMAC, 
        "Set bits[4:0]=0xD miscPllCtrl<0x%p> = 0x%x", 
        &MISC->miscPllCtrl, (unsigned int) MISC->miscPllCtrl );

    /* Enable GMAC clock */
    PERF->blkEnables |= GMAC_UBUS_CLK_EN;

    BCM_LOG_DEBUG( BCM_LOG_ID_GMAC, 
        "Enable GMAC clock (bit30=0x40000000) blkEnables<0x%p>=0x%x", 
        &PERF->blkEnables, (unsigned int) PERF->blkEnables );

    MISC->miscIddqCtrl &= ~0x10000;

    BCM_LOG_DEBUG( BCM_LOG_ID_GMAC, 
        "Cleared IDDQ bit (0x10000) miscIddqCtrl<0x%p> = 0x%x", 
        &MISC->miscIddqCtrl, (unsigned int) MISC->miscIddqCtrl );

    /* Now mux GMAC to Port 4 of RoboSwitch */
    BCM_LOG_DEBUG( BCM_LOG_ID_GMAC, "Select GMAC at Mux (bit22=0x400000)" ); 
    GPIO->GPIOBaseMode |= EN_SEL_GMAC_VS_GPON;

    BCM_LOG_DEBUG( BCM_LOG_ID_GMAC, "\tGPIOBaseMode<0x%p>=0x%x", 
        &GPIO->GPIOBaseMode, (uint32_t) GPIO->GPIOBaseMode );
}
#endif
...
void gmac_init_default( void )
{
    ...
    gmac_reset_mib();
    gmac_intf_set_mib_max_pkt_size( GMAC_MAX_FRM_LEN );
    gmac_mac_set_max_frm_len( GMAC_MAX_FRM_LEN );
