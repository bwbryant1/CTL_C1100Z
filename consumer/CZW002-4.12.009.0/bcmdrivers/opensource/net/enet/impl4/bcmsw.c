...
#ifdef BUILD_HPNA 
void auto_set_pause_capability(int port, char* ifname)
{
	uint16 lpa = 0;
    ...
    ethsw_phy_rreg(ETHSW_PHY_GET_PHYID(port), MII_LPA, &lpa);
    if(lpa & ADVERTISE_PAUSE_CAP || port == HPNA_PORTS_ID) {
		if (port != HPNA_PORTS_ID) // remove redundant message to avoid error capture result on TE testing
        printk(KERN_CRIT "Enable Flow Control on %s port %d\n", (ifname==NULL?"":ifname), port);
        override_val |= ((1 << port) | (1 << (port +TOTAL_SWITCH_PORTS)));
    }
	else {
		if (port != HPNA_PORTS_ID) // remove redundant message to avoid error capture result on TE testing
        printk(KERN_CRIT "Disable Flow Control on %s port %d\n", (ifname==NULL?"":ifname), port);
    }
...
void ethsw_set_wanoe_portmap(uint16 wan_port_map)
{
	...
	//Can't forward packets from external switch to internal port. CSP 581301
	//The MAC learning table is incorrect
	fast_age_all(1);
/*
...
#if 1 //__CTLK__, Kaiping, Ether LAN statistics, Get internal/external switch mib count, dump external switch mib from 4.16L03
int ethsw_get_mib(int port, struct ethswctl_data *eth_data)
{
    volatile EthSwMIBRegs *e;
    int base;
    int bExt6829 = 0;
    ...
    if ( 0 == bExt6829 )
    {
        ...
        if (gmac_info_pg->enabled && (port == GMAC_PORT_ID) )
        {
            ...
            /* Display remaining rx stats only if requested */
            eth_data->rxJabbers = e->RxJabbers + g->RxJabbers;
            eth_data->rxFragments = e->RxFragments + g->RxFragments;
            eth_data->rxOversize = e->RxOversizePkts + g->RxOversizePkts;
            eth_data->rxUndersize = e->RxUndersizePkts + g->RxUndersizePkts;
            return 0;
        }
#endif
        eth_data->rxJabbers = e->RxJabbers;
        eth_data->rxFragments = e->RxFragments;
        eth_data->rxOversize = e->RxOversizePkts;
        eth_data->rxUndersize = e->RxUndersizePkts;
    }
    else
    {
        panic("ethsw_get_mib: External SPI slave not available\n");
    }

    return 0;
}

int ethsw_dump_mib_ext(int port, int type)
{
    ...
    extsw_rreg_wrap(PAGE_MIB_P0 + (port), REG_MIB_P0_EXT_TXUPKTS, (uint8_t *)&v32, 4);  // Get TX unicast packet count
    printk("TxUnicastPkts:          %10u \n", v32);
    extsw_rreg_wrap(PAGE_MIB_P0 + (port), REG_MIB_P0_EXT_TXMPKTS, (uint8_t *)&v32, 4);  // Get TX multicast packet count
    printk("TxMulticastPkts:        %10u \n",  v32);
    extsw_rreg_wrap(PAGE_MIB_P0 + (port), REG_MIB_P0_EXT_TXBPKTS, (uint8_t *)&v32, 4);  // Get TX broadcast packet count
    printk("TxBroadcastPkts:        %10u \n", v32);
    extsw_rreg_wrap(PAGE_MIB_P0 + (port), REG_MIB_P0_EXT_TXDROPS, (uint8_t *)&v32, 4);
    printk("TxDropPkts:             %10u \n", v32);
    ...
    if (type)
    {
        ...
        //v32 = (((uint32*)data)[1]);
        //printk("TxOctetsHi:             %10u \n", v32);
        extsw_rreg_wrap(PAGE_MIB_P0 + (port), REG_MIB_P0_EXT_TXQ0PKT, (uint8_t *)&v32, 4);
        printk("TxQ0Pkts:               %10u \n", v32);
        extsw_rreg_wrap(PAGE_MIB_P0 + (port), REG_MIB_P0_EXT_TXQ1PKT, (uint8_t *)&v32, 4);
        printk("TxQ1Pkts:               %10u \n", v32);
        extsw_rreg_wrap(PAGE_MIB_P0 + (port), REG_MIB_P0_EXT_TXQ2PKT, (uint8_t *)&v32, 4);
        printk("TxQ2Pkts:               %10u \n", v32);
        extsw_rreg_wrap(PAGE_MIB_P0 + (port), REG_MIB_P0_EXT_TXQ3PKT, (uint8_t *)&v32, 4);
        printk("TxQ3Pkts:               %10u \n", v32);
        extsw_rreg_wrap(PAGE_MIB_P0 + (port), REG_MIB_P0_EXT_TXQ4PKT, (uint8_t *)&v32, 4);
        printk("TxQ4Pkts:               %10u \n", v32);
        extsw_rreg_wrap(PAGE_MIB_P0 + (port), REG_MIB_P0_EXT_TXQ5PKT, (uint8_t *)&v32, 4);
        printk("TxQ5Pkts:               %10u \n", v32);
        extsw_rreg_wrap(PAGE_MIB_P0 + (port), REG_MIB_P0_EXT_TXCOL, (uint8_t *)&v32, 4);
        printk("TxCol:                  %10u \n", v32);
        extsw_rreg_wrap(PAGE_MIB_P0 + (port), REG_MIB_P0_EXT_TXSINGLECOL, (uint8_t *)&v32, 4);
        printk("TxSingleCol:            %10u \n", v32);
        extsw_rreg_wrap(PAGE_MIB_P0 + (port), REG_MIB_P0_EXT_TXMULTICOL, (uint8_t *)&v32, 4);
        printk("TxMultipleCol:          %10u \n", v32);
        extsw_rreg_wrap(PAGE_MIB_P0 + (port), REG_MIB_P0_EXT_TXDEFERREDTX, (uint8_t *)&v32, 4);
        printk("TxDeferredTx:           %10u \n", v32);
        extsw_rreg_wrap(PAGE_MIB_P0 + (port), REG_MIB_P0_EXT_TXLATECOL, (uint8_t *)&v32, 4);
        printk("TxLateCol:              %10u \n", v32);
        extsw_rreg_wrap(PAGE_MIB_P0 + (port), REG_MIB_P0_EXT_TXEXCESSCOL, (uint8_t *)&v32, 4);
        printk("TxExcessiveCol:         %10u \n", v32);
        extsw_rreg_wrap(PAGE_MIB_P0 + (port), REG_MIB_P0_EXT_TXFRAMEINDISC, (uint8_t *)&v32, 4);
        printk("TxFrameInDisc:          %10u \n", v32);
        extsw_rreg_wrap(PAGE_MIB_P0 + (port), REG_MIB_P0_EXT_TXPAUSEPKTS, (uint8_t *)&v32, 4);
        printk("TxPausePkts:            %10u \n", v32);
    }
    else
    {
    ...
    /* Display Rx statistics */
    extsw_rreg_wrap(PAGE_MIB_P0 + (port), REG_MIB_P0_EXT_RXUPKTS, (uint8_t *)&v32, 4);  // Get RX unicast packet count
    printk("RxUnicastPkts:          %10u \n", v32);
    extsw_rreg_wrap(PAGE_MIB_P0 + (port), REG_MIB_P0_EXT_RXMPKTS, (uint8_t *)&v32, 4);  // Get RX multicast packet count
    printk("RxMulticastPkts:        %10u \n",v32);
    extsw_rreg_wrap(PAGE_MIB_P0 + (port), REG_MIB_P0_EXT_RXBPKTS, (uint8_t *)&v32, 4);  // Get RX broadcast packet count
    printk("RxBroadcastPkts:        %10u \n",v32);
    extsw_rreg_wrap(PAGE_MIB_P0 + (port), REG_MIB_P0_EXT_RXDROPS, (uint8_t *)&v32, 4);
    printk("RxDropPkts:             %10u \n",v32);
    extsw_rreg_wrap(PAGE_MIB_P0 + (port), REG_MIB_P0_EXT_RXDISCARD, (uint8_t *)&v32, 4);
    ...
    if (type)
    {
        ...
        //v32 = (((uint32*)data)[1]);
        //printk("RxGoodOctetsHi:         %10u \n", v32);
        extsw_rreg_wrap(PAGE_MIB_P0 + (port), REG_MIB_P0_EXT_RXJABBERS, (uint8_t *)&v32, 4);
        printk("RxJabbers:              %10u \n", v32);
        extsw_rreg_wrap(PAGE_MIB_P0 + (port), REG_MIB_P0_EXT_RXALIGNERRORS, (uint8_t *)&v32, 4);
        printk("RxAlignErrs:            %10u \n", v32);
        extsw_rreg_wrap(PAGE_MIB_P0 + (port), REG_MIB_P0_EXT_RXFCSERRORS, (uint8_t *)&v32, 4);
        printk("RxFCSErrs:              %10u \n", v32);
        extsw_rreg_wrap(PAGE_MIB_P0 + (port), REG_MIB_P0_EXT_RXFRAGMENTS, (uint8_t *)&v32, 4);
        printk("RxFragments:            %10u \n", v32);
        extsw_rreg_wrap(PAGE_MIB_P0 + (port), REG_MIB_P0_EXT_RXOVERSIZE, (uint8_t *)&v32, 4);
        printk("RxOversizePkts:         %10u \n", v32);
        extsw_rreg_wrap(PAGE_MIB_P0 + (port), REG_MIB_P0_EXT_RXUNDERSIZEPKTS, (uint8_t *)&v32, 4);
        printk("RxUndersizePkts:        %10u \n", v32);
        extsw_rreg_wrap(PAGE_MIB_P0 + (port), REG_MIB_P0_EXT_RXPAUSEPKTS, (uint8_t *)&v32, 4);
        printk("RxPausePkts:            %10u \n", v32);
        extsw_rreg_wrap(PAGE_MIB_P0 + (port), REG_MIB_P0_EXT_RXSACHANGES, (uint8_t *)&v32, 4);
        printk("RxSAChanges:            %10u \n", v32);
        extsw_rreg_wrap(PAGE_MIB_P0 + (port), REG_MIB_P0_EXT_RXSYMBOLERRORS, (uint8_t *)&v32, 4);
        printk("RxSymbolError:          %10u \n", v32);
        extsw_rreg_wrap(PAGE_MIB_P0 + (port), REG_MIB_P0_EXT_RX64OCTPKTS, (uint8_t *)&v32, 4);
        printk("RxPkts64Octets:         %10u \n", v32);
        extsw_rreg_wrap(PAGE_MIB_P0 + (port), REG_MIB_P0_EXT_RX127OCTPKTS, (uint8_t *)&v32, 4);
        printk("RxPkts65to127Octets:    %10u \n", v32);
        extsw_rreg_wrap(PAGE_MIB_P0 + (port), REG_MIB_P0_EXT_RX255OCTPKTS, (uint8_t *)&v32, 4);
        printk("RxPkts128to255Octets:   %10u \n", v32);
        extsw_rreg_wrap(PAGE_MIB_P0 + (port), REG_MIB_P0_EXT_RX511OCTPKTS, (uint8_t *)&v32, 4);
        printk("RxPkts256to511Octets:   %10u \n", v32);
        extsw_rreg_wrap(PAGE_MIB_P0 + (port), REG_MIB_P0_EXT_RX1023OCTPKTS, (uint8_t *)&v32, 4);
        printk("RxPkts512to1023Octets:  %10u \n", v32);
        extsw_rreg_wrap(PAGE_MIB_P0 + (port), REG_MIB_P0_EXT_RXMAXOCTPKTS, (uint8_t *)&v32, 4);
        printk("RxPkts1024OrMoreOctets: %10u \n", v32);
        extsw_rreg_wrap(PAGE_MIB_P0 + (port), REG_MIB_P0_EXT_RXJUMBOPKT , (uint8_t *)&v32, 4);
        printk("RxJumboPkts:            %10u \n", v32);
        extsw_rreg_wrap(PAGE_MIB_P0 + (port), REG_MIB_P0_EXT_RXOUTRANGEERR, (uint8_t *)&v32, 4);
        printk("RxOutOfRange:           %10u \n", v32);
        extsw_rreg_wrap(PAGE_MIB_P0 + (port), REG_MIB_P0_EXT_RXINRANGEERR, (uint8_t *)&v32, 4);
...
int ethsw_get_mib_ext(int port, struct ethswctl_data *eth_data)
{
    unsigned int v32;

    extsw_rreg_wrap(PAGE_MIB_P0 + (port), REG_MIB_P0_EXT_RXJABBERS, (uint8_t *)&v32, 4);
    eth_data->rxJabbers = v32;
    
    extsw_rreg_wrap(PAGE_MIB_P0 + (port), REG_MIB_P0_EXT_RXFRAGMENTS, (uint8_t *)&v32, 4);
    eth_data->rxFragments = v32;

    extsw_rreg_wrap(PAGE_MIB_P0 + (port), REG_MIB_P0_EXT_RXOVERSIZE, (uint8_t *)&v32, 4);
    eth_data->rxOversize = v32;

    extsw_rreg_wrap(PAGE_MIB_P0 + (port), REG_MIB_P0_EXT_RXUNDERSIZEPKTS, (uint8_t *)&v32, 4);
    eth_data->rxUndersize = v32;
    return 0;
}
#endif
