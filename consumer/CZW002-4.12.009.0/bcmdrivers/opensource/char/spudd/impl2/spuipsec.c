...
extern int spu_avail_input_desc (void);
                              ...
                              u_int16_t * spdSize)
{
	...
	*supdtFrag = sizeof (SCTX_OUTSAU);
	...
	default:
	    status = BCM_STATUS_INVALID_INPUT;
	    printk (KERN_ERR "BcmCreateOperationContext bad mode %x\n", 
                              op->flags.mode);
    }
	...
	default:
	    status = BCM_STATUS_INVALID_INPUT;
	    printk (KERN_ERR "BcmCreateOperationContext bad authAlg %x\n",
                              op->authAlg);
    }
	...
	case BCMSAD_ENCR_ALG_AES_CTR:
		...
		default:
		    status = BCM_STATUS_INVALID_INPUT;
		    printk (KERN_ERR "BcmCreateOperationContext bad AES keylen %d\n",
                                      op->encrKeyLen);
	    }
	    break;
	...
	default:
	    status = BCM_STATUS_INVALID_INPUT;
	    printk (KERN_ERR "BcmCreateOperationContext bad encrAlg %x\n", 
                              op->encrAlg);
    }
                             ...
                             SACTX * devsa)
{
     ...
     */
    if (spu_avail_input_desc () < (nfrags + 2))
    {
        printk (KERN_ERR "Too many fragments for input\n");
        return BCM_STATUS_RESOURCE;
    }
...
int spu_process_ipsec(struct spu_trans_req *trans_req, BCM_CRYPTOOP *op)
{
    ...
    /* allocate an additional 4 bytes for the status word and account for 
       alignment */
    trans_req->cmd_buf = (void *)kmalloc(BCM_XTRA_DMA_HDR_SIZE + ALIGN_SIZE + 4, GFP_ATOMIC);
    if (trans_req->cmd_buf == NULL)
    {
        printk(KERN_ERR "Allocate DMA Context Memory error\n");
        goto bcm_error1;
    }
    spu_cmd_ptr = (unsigned char *)IPSEC_SPU_ALIGN (trans_req->cmd_buf, ALIGN_SIZE);
...
#if 0
    status = spu_sa_init (spu_cmd_ptr, 
				    &spu_cmd_offset, op, insau, &sauSize);
    if(status != BCM_STATUS_OK)
    {
...
bcm_error1:
    ...
    if(trans_req->cmd_buf)
    {
        kfree(trans_req->cmd_buf);
        trans_req->cmd_buf = NULL;
    }
...
#endif
{
    ...
    kfree(spu_req->cmd_buf);
    spu_req->cmd_buf = NULL;
